...
- `symbolic web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `symbolic web` view. **Do not** add for terminal, TUI, or general symbolic issues.
...
**Only** add if the issue mentions "zen" or "symbolic zen" or "symbolic black".

If the issue doesn't have "zen" or "symbolic black" in it then don't add zen label

#### core

Use for core server issues in `packages/symbolic/`, excluding `packages/symbolic/src/cli/cmd/tui/`.
...
TUI (`packages/symbolic/src/cli/cmd/tui/...`):
...
Core (`packages/symbolic/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for symbolic server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
