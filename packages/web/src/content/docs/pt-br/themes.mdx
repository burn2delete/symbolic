...
Com o symbolic, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o symbolic usa nosso próprio tema `symbolic`.
...
O symbolic vem com vários temas embutidos.
...
- Querem que o symbolic corresponda à aparência do seu terminal
...
O symbolic suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/symbolic/themes/*.json` ou `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.symbolic/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.symbolic/themes/*.json`
