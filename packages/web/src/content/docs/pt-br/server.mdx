---
title: Servidor
description: Interaja com o servidor symbolic via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `symbolic serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente symbolic pode usar.
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `symbolic.local` |
...
Defina `SYMBOLIC_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `symbolic`, ou defina `SYMBOLIC_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `symbolic serve` quanto ao `symbolic web`.

```bash
SYMBOLIC_SERVER_PASSWORD=your-password symbolic serve
```
...
Quando você executa `symbolic`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor symbolic para interagir com o symbolic programaticamente.
:::

Essa arquitetura permite que o symbolic suporte múltiplos clientes e permite que você interaja com o symbolic programaticamente.

Você pode executar `symbolic serve` para iniciar um servidor autônomo. Se você tiver o TUI do symbolic em execução, `symbolic serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do symbolic [IDE](/docs/ide).
...
O servidor symbolic expõe as seguintes APIs.
