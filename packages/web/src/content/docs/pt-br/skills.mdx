...
As habilidades do agente permitem que o symbolic descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O symbolic pesquisa nesses locais:

- Configuração do projeto: `.symbolic/skills/<name>/SKILL.md`
- Configuração global: `~/.config/symbolic/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o symbolic sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.symbolic/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/symbolic/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.symbolic/skills/git-release/SKILL.md` assim:
...
O symbolic lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `symbolic.json`:
...
**Para agentes embutidos** (em `symbolic.json`):
