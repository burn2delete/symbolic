---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w symbolic.
---

import config from "../../../../config.mjs"
export const console = config.console

symbolic używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji symbolic.
...
w `~/.local/share/symbolic/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim symbolic
config.
...
Symbolic Zen to lista modeli dostarczonych przez zespół symbolic, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z symbolic. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Symbolic Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Symbolic Zen` i przejdź do [symbolic.computer/auth](https://symbolic.computer/zen).
...
Działa jak każdy inny dostawca w symbolic i jest całkowicie opcjonalny w użyciu.

---

## Symbolic Go

Symbolic Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół symbolic, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z symbolic.

1. Uruchom polecenie `/connect` w TUI, wybierz `Symbolic Go` i przejdź do [symbolic.computer/auth](https://symbolic.computer/zen).
...
Aby używać Amazon Bedrock z symbolic:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania symbolic:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `symbolic.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w symbolic nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod symbolic działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji symbolic.
...
symbolic używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować symbolic
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`symbolic.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Uwierzytelnianie GitLab jest wbudowane w Symbolic. Zobacz sekcję dostawcy GitLab powyżej.

##### Konfiguracja

Customize through `symbolic.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem symbolic:
...
Aby używać Google Vertex AI z symbolic:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania symbolic.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą symbolic, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session), aby automatycznie rejestrować każdą konwersację symbolic jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja symbolic będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować symbolic tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować symbolic tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować symbolic tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem symbolic. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/symbolic).
:::
...
Aby korzystać z Ollama Cloud z symbolic:
...
3. Skopiuj klucz API do użycia w symbolic.
...
6. **Ważne**: Przed użyciem modeli chmurowych w symbolic musisz pobrać informacje o modelu lokalnie:
...
Symbolic Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół symbolic. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Symbolic Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Symbolic Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API symbolic.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji symbolic.
...
5. Możesz je także dostosować za pomocą konfiguracji symbolic. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z symbolic:
...
Możesz także dostosować modele za pomocą konfiguracji symbolic. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z symbolic. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `symbolic.json` w katalogu projektu:
...
Pola `limit` pozwalają symbolic zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `symbolic auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację symbolic i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji symbolic.
