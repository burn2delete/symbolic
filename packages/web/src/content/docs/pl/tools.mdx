...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Symbolic posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Symbolic.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `SYMBOLIC_EXPERIMENTAL_LSP_TOOL=true` (lub `SYMBOLIC_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Symbolic lub gdy zmienna środowiskowa `SYMBOLIC_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Symbolic:
