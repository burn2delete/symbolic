...
Dzięki symbolic możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie symbolic używa naszego własnego motywu `symbolic`.
...
symbolic ma kilka wbudowanych motywów.
...
- Chcą, aby Symbolic pasował do wyglądu ich terminala
...
symbolic obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/symbolic/themes/*.json` lub `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.symbolic/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.symbolic/themes/*.json`
