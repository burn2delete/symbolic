...
Możesz dodać dodatkowe informacje do symbolic za pomocą _Model Context Protocol_, w skrócie MCP. symbolic obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [symbolic Config](https://symbolic.computer/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/symbolic`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
symbolic automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, symbolic:
...
Jeśli serwer wymaga uwierzytelnienia, symbolic poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `symbolic mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji symbolic bezpieczne przechowa tokeny w `~/.local/share/symbolic/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w symbolic, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie symbolic, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć symbolic z kontem Sentry.
