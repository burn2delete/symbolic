...
Umiejętności agenta pozwalają symbolic odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
symbolic przeszukuje te lokalizacje:

- Project config: `.symbolic/skills/<name>/SKILL.md`
- Global config: `~/.config/symbolic/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu symbolic przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.symbolic/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/symbolic/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.symbolic/skills/git-release/SKILL.md` w ten sposób:
...
symbolic wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `symbolic.json`:
...
**Dla agentów wbudowanych** (w `symbolic.json`):
