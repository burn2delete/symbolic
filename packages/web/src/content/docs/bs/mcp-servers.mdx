...
Možete dodati vanjske alate u Symbolic koristeći _Model Context Protocol_, ili MCP. Symbolic podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Symbolic Config](https://symbolic.computer/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/symbolic`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Symbolic automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Symbolic će:
...
Ako server zahtijeva autentifikaciju, Symbolic će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `symbolic mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Symbolic sigurno čuva tokene u `~/.local/share/symbolic/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Symbolic, zajedno s ugrađenim alatima. Možete njima upravljati kroz Symbolic konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Symbolic sa vašim Sentry nalogom.
