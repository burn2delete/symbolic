---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Symbolic.
---

import config from "../../../../config.mjs"
export const console = config.console

Symbolic koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Symbolic konfiguraciji.
...
u `~/.local/share/symbolic/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Symbolic
config.
...
Symbolic Zen je lista modela koje je obezbedio Symbolic tim koji su bili
testirano i potvrđeno da dobro radi sa Symbolic. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Symbolic Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite symbolic i idite na [symbolic.computer/auth](https://symbolic.computer/auth).
...
Radi kao i svaki drugi provajder u Symbolic i potpuno je opcionalan za korištenje.

---

## Symbolic Go

Symbolic Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Symbolic tim i koji su testirani i verificirani da dobro rade s Symbolic-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Symbolic Go` i idite na [symbolic.computer/auth](https://symbolic.computer/zen).
...
Da biste koristili Amazon Bedrock s Symbolic:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete symbolic:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `symbolic.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Symbolic.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi symbolic ispravno radio.
   :::
...
Također možete dodati modele kroz svoju symbolic konfiguraciju.
...
da pohrani token u symbolic auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Symbolic koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Symbolic
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`symbolic.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
GitLab autentifikacija je ugrađena u Symbolic. Pogledajte GitLab provider dokumentaciju iznad.

##### Konfiguracija

Prilagodite putem `symbolic.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s symbolic:
...
Za korištenje Google Vertex AI s Symbolic:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće symbolic.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem symbolica, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session) da automatski prijavite svaki Symbolic razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Symbolic razgovor naveden kao zasebna sesija.
...
Možete konfigurirati symbolic za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati symbolic za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati symbolic za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Symbolic. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/symbolic) za detalje.
:::
...
Da biste koristili Ollama Cloud s Symbolic:
...
3. Kopirajte API ključ za korištenje u Symbolic.
...
6. **Važno**: Prije upotrebe modela oblaka u Symbolic, morate lokalno povući informacije o modelu:
...
Symbolic Zen je lista testiranih i verifikovanih modela koju je obezbedio Symbolic tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Symbolic Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Symbolic Zen**.

```txt
   /connect
```

3. Unesite svoj Symbolic API ključ.
...
Također možete dodati dodatne modele putem vaše symbolic konfiguracije.
...
5. Također ih možete prilagoditi putem vaše symbolic konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Symbolicom:
...
Također možete prilagoditi modele kroz svoju symbolic konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s symbolic-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `symbolic.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Symbolic da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `symbolic auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Symbolic konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj symbolic konfiguraciji.
