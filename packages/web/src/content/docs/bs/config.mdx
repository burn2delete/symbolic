---
title: Konfiguracija
description: Korištenje Symbolic JSON konfiguracije.
---

Možete konfigurirati Symbolic koristeći JSON konfiguracijski fajl.

---

## Format

Symbolic podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/symbolic`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/symbolic/symbolic.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`SYMBOLIC_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`symbolic.json` u projektu) - postavke specifične za projekat
5. **`.symbolic` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`SYMBOLIC_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.symbolic` i `~/.config/symbolic` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/symbolic` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Symbolic konfiguraciju u `~/.config/symbolic/symbolic.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/symbolic/tui.json`.
...
Dodajte `symbolic.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Symbolic pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `SYMBOLIC_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `SYMBOLIC_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.symbolic` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.symbolic`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`symbolic.computer/config.json`**](https://symbolic.computer/config.json).

TUI konfiguracija koristi [**`symbolic.computer/tui.json`**](https://symbolic.computer/tui.json).
...
Koristite `SYMBOLIC_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `symbolic.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `symbolic serve` i `symbolic web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Symbolic server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `symbolic.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Symbolic konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Symbolic pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/symbolic/agents/` ili `.symbolic/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Symbolic će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`symbolic run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/symbolic/commands/` ili `.symbolic/commands/`. [Saznajte više ovdje](/docs/commands).
...
Symbolic će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Symbolic **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Symbolic sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.symbolic/plugins/` ili `~/.config/symbolic/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Symbolic da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
