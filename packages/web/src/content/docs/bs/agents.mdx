...
Postoje dvije vrste agenata u Symbolic: primarni agenti i podagenti.
...
Symbolic dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Symbolic dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Symbolic dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `symbolic.json`:
...
- Globalno: `~/.config/symbolic/agents/`
- Po projektu: `.symbolic/agents/`
...
Ako temperatura nije navedena, Symbolic koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Symbolic konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Symbolic konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Symbolic Zen](/docs/zen), koristili biste `symbolic/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `symbolic models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/SymbolicOS/symbolic).
:::
