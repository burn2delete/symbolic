---
title: Server
description: Nutze den symbolic-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `symbolic serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein symbolic-Client nutzen kann.
...
Setze `SYMBOLIC_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `symbolic`, kann aber mit `SYMBOLIC_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `symbolic serve` und `symbolic web`.
...
Wenn du `symbolic` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den symbolic-Server, um symbolic programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `symbolic serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `symbolic serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Symbolic-[IDE](/docs/ide)-Plugins.
...
Der symbolic-Server stellt folgende APIs bereit.
