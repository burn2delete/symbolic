---
title: Regeln
description: Lege eigene Anweisungen fuer symbolic fest.
---

Du kannst symbolic ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in symbolic den Befehl `/init` aus.
...
So kann symbolic sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht symbolic sie zu erweitern.
...
symbolic kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/symbolic/AGENTS.md` speichern.
Diese gelten in allen symbolic-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Symbolic dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/symbolic/AGENTS.md` existiert)
...
Beim Start sucht symbolic in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/symbolic/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `symbolic.json` oder in `~/.config/symbolic/symbolic.json` angeben.
...
symbolic parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit symbolic.json

Empfohlen ist das `instructions`-Feld in der `symbolic.json`:
...
Du kannst symbolic explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `symbolic.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
