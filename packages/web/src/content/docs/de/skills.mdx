...
Agent Skills erlauben Symbolic, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Symbolic sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Symbolic vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.symbolic/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/symbolic/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.symbolic/skills/git-release/SKILL.md` zum Beispiel so:
...
Symbolic listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `symbolic.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `symbolic.json`):
