...
Symbolic Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte Open-Coding-Modelle bietet.

:::note
Symbolic Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Anbieter in Symbolic. Du abonnierst Symbolic Go und erhältst deinen API-Schlüssel. Es ist **völlig optional** und du musst es nicht nutzen, um Symbolic zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Anbietern getestet, die gut mit Symbolic funktionieren.
:::
...
Symbolic Go gibt dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

Symbolic Go funktioniert wie jeder andere Anbieter in Symbolic.

1. Du meldest dich bei **<a href={console}>Symbolic Zen</a>** an, abonnierst Go und kopierst deinen API-Schlüssel.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Symbolic Go` und fügst deinen API-Schlüssel ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Symbolic Go abonnieren.
:::
...
Symbolic Go beinhaltet die folgenden Limits:
...
Symbolic Go ist ein Abonnementplan für **10 $/Monat**. Unten stehen die Preise **pro 1 Mio. Token**.
...
Die [Modell-ID](/docs/config/#models) in deiner Symbolic-Konfiguration verwendet das Format `symbolic-go/<model-id>`. Zum Beispiel würdest du für Kimi K2.5 `symbolic-go/kimi-k2.5` in deiner Konfiguration verwenden.
...
Wir haben Symbolic Go erstellt, um:
...
4. **Keinen Lock-in** zu haben, indem wir dir ermöglichen, jeden anderen Anbieter ebenfalls mit Symbolic zu nutzen.
