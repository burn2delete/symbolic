...
Puoi aggiungere strumenti esterni a Symbolic usando il _Model Context Protocol_, o MCP. Symbolic supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Symbolic Config](https://symbolic.computer/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/symbolic`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Symbolic gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Symbolic:
...
Se il server richiede autenticazione, Symbolic ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `symbolic mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Symbolic memorizzera' i token in modo sicuro in `~/.local/share/symbolic/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Symbolic insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Symbolic come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Symbolic al tuo account Sentry.
