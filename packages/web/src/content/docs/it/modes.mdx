...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di symbolic. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in symbolic ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di symbolic.
...
symbolic include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.symbolic/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `symbolic.json`:
...
- Globale: `~/.config/symbolic/modes/`
- Progetto: `.symbolic/modes/`
...
Se non viene specificata alcuna temperatura, symbolic usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di symbolic sia per quella specifica del progetto.
...
Crea file di modalita in `.symbolic/modes/` per modalita specifiche del progetto oppure in `~/.config/symbolic/modes/` per modalita globali:
