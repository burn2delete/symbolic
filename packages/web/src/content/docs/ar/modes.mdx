...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات symbolic. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في symbolic تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات symbolic.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.symbolic/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `symbolic.json` لديك:
...
- عالمي: `~/.config/symbolic/modes/`
- للمشروع: `.symbolic/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم symbolic القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات symbolic العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.symbolic/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/symbolic/modes/` لأوضاع عالمية:
