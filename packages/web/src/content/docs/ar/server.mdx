---
title: الخادم
description: تفاعل مع خادم symbolic عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `symbolic serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل symbolic استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `symbolic.local` |
...
عيّن `SYMBOLIC_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `symbolic`، أو عيّن `SYMBOLIC_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `symbolic serve` و `symbolic web`.
...
عند تشغيل `symbolic` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم symbolic للتفاعل مع symbolic برمجيا.
:::

تتيح هذه البنية لـ symbolic دعم عدة عملاء وتمكّنك من التفاعل مع symbolic برمجيا.

يمكنك تشغيل `symbolic serve` لبدء خادم مستقل. إذا كانت واجهة symbolic في terminal (TUI)
قيد التشغيل، فسيبدأ `symbolic serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Symbolic لـ [IDE](/docs/ide).
...
يعرض خادم symbolic واجهات API التالية.
