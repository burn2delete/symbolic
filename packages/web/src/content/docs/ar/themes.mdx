...
مع Symbolic يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Symbolic سمتنا `symbolic`.
...
يأتي Symbolic مع عدة سمات مدمجة.
...
- يريدون أن يطابق Symbolic مظهر terminal لديهم
...
يدعم Symbolic نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/symbolic/themes/*.json` أو `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.symbolic/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.symbolic/themes/*.json`
