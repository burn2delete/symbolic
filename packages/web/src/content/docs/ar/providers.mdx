---
title: المزوّدون
description: استخدام أي موفّر LLM في Symbolic.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Symbolic ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Symbolic.
...
في `~/.local/share/symbolic/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Symbolic.
...
Symbolic Zen هي قائمة نماذج يوفّرها فريق Symbolic وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Symbolic. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Symbolic Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر symbolic، ثم انتقل إلى [symbolic.computer/auth](https://symbolic.computer/auth).
...
يعمل مثل أي مزوّد آخر في Symbolic واستخدامه اختياري بالكامل.

---

## Symbolic Go

Symbolic Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Symbolic، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Symbolic.

1. شغّل الأمر `/connect` في TUI، واختر `Symbolic Go`، ثم انتقل إلى [symbolic.computer/auth](https://symbolic.computer/zen).
...
لاستخدام Amazon Bedrock مع Symbolic:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل symbolic:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `symbolic.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Symbolic غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل symbolic بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات symbolic.
...
تخزين الرمز في مساحة تخزين مصادقة symbolic.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Symbolic نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Symbolic
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`symbolic.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مصادقة GitLab مدمجة في Symbolic. راجع وثائق موفر GitLab أعلاه.

##### التهيئة

خصّص عبر `symbolic.json`:
...
لاستخدام اشتراك GitHub Copilot مع symbolic:
...
لاستخدام Google Vertex AI مع Symbolic:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر symbolic، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session) لتسجيل كل محادثة Symbolic تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Symbolic مدرجة كجلسة مستقلة.
...
يمكنك ضبط symbolic لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط symbolic لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط symbolic لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Symbolic. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/symbolic) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Symbolic:
...
3. انسخ مفتاح API لاستخدامه في Symbolic.
...
6. **مهم**: قبل استخدام نماذج السحابة في Symbolic، يجب سحب معلومات النموذج محليا:
...
Symbolic Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Symbolic. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Symbolic Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Symbolic Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Symbolic.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات symbolic.
...
5. يمكنك أيضا تخصيصها عبر إعدادات symbolic. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Symbolic:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات symbolic. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع symbolic. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `symbolic.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Symbolic بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `symbolic auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات symbolic وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات symbolic.
