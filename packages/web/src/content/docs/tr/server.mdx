---
title: Sunucu
description: symbolic sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`symbolic serve` komutu, symbolic istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `symbolic.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `SYMBOLIC_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `symbolic` değeridir; değiştirmek isterseniz `SYMBOLIC_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `symbolic serve` hem de `symbolic web` için geçerlidir.
...
`symbolic` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
symbolic ile programatik etkileşim için symbolic sunucusunu kullanın.
:::

Bu mimari, symbolic'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`symbolic serve` ile bağımsız bir sunucu başlatabilirsiniz. symbolic TUI açıksa, `symbolic serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum symbolic [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
symbolic sunucusu aşağıdaki API'leri sunar.
