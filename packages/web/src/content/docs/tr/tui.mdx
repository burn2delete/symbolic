---
title: TUI
description: Symbolic terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Symbolic, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

Symbolic'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
Symbolic TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
Symbolic'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
Symbolic'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `symbolic.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `SYMBOLIC_TUI_CONFIG` kullanın.
