...
symbolic ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak symbolic kendi `symbolic` temasını kullanır.
...
symbolic birden fazla yerleşik temayla gelir.
...
- symbolic'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
symbolic, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/symbolic/themes/*.json` veya `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.symbolic/themes/*.json`
4. **Geçerli çalışma dizini** - `./.symbolic/themes/*.json`
