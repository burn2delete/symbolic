...
_Model Bağlam Protokolü_ veya MCP kullanarak symbolic'a harici araçlar ekleyebilirsiniz. symbolic hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [symbolic Config](https://symbolic.computer/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/symbolic` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
symbolic, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde symbolic şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa symbolic, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `symbolic mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından symbolic, belirteçleri `~/.local/share/symbolic/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra symbolic'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi symbolic yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve symbolic'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
