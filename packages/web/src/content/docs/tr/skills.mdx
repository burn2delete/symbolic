...
Ajan becerileri, symbolic'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
symbolic su konumlari tarar:

- Proje konfigurasyonu: `.symbolic/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/symbolic/skills/<name>/SKILL.md`
...
Proje yerel yollarinda symbolic, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.symbolic/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/symbolic/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.symbolic/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
symbolic, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`symbolic.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`symbolic.json` icinde):
