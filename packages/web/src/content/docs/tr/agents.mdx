...
symbolic'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
symbolic, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
symbolic, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

symbolic iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `symbolic.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse symbolic modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global symbolic yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
symbolic yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Symbolic Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `symbolic/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `symbolic models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/SymbolicOS/symbolic).
:::
