...
description: Symbolic, LSP sunucularınızla bütünleşir.
---

Symbolic, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

Symbolic, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`SYMBOLIC_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
symbolic bir dosyayı açtığında:
...
LSP sunucularını, symbolic yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.
