...
Symbolic Go, popüler açık kodlama modellerine güvenilir erişim sağlayan **aylık 10$** tutarında düşük maliyetli bir aboneliktir.

:::note
Symbolic Go şu anda beta aşamasındadır.
:::

Go, Symbolic içindeki diğer sağlayıcılar gibi çalışır. Symbolic Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Symbolic'u kullanmak için buna ihtiyacınız yoktur.
...
:::tip
Symbolic ile iyi çalışan seçkin bir model ve sağlayıcı grubunu test ettik.
:::
...
Symbolic Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır

Symbolic Go, Symbolic'daki diğer herhangi bir sağlayıcı gibi çalışır.

1. **<a href={console}>Symbolic Zen</a>**'de oturum açın, Go'ya abone olun ve API anahtarınızı kopyalayın.
...
:::note
Çalışma alanı başına yalnızca bir üye Symbolic Go'ya abone olabilir.
:::
...
Symbolic Go aşağıdaki sınırları içerir:
...
Symbolic Go, **aylık 10$** tutarında bir abonelik planıdır. Aşağıda **1M token başına** fiyatlar yer almaktadır.
...
Symbolic yapılandırmanızdaki [model kimliği](/docs/config/#models), `symbolic-go/<model-id>` biçimini kullanır. Örneğin, Kimi K2.5 için yapılandırmanızda `symbolic-go/kimi-k2.5` kullanırsınız.
...
4. Symbolic ile başka herhangi bir sağlayıcıyı kullanmanıza da izin vererek **kilitlenmeyi önlemek**.
