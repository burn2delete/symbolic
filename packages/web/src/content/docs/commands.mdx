...
Create `.symbolic/commands/test.md`:
...
You can add custom commands through the Symbolic config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Symbolic [config](/docs/config):
...
symbolic includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
