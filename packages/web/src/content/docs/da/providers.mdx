---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Symbolic.
---

import config from "../../../../config.mjs"
export const console = config.console

Symbolic bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Symbolic-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/symbolic/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Symbolic-konfigurationen.
...
Symbolic Zen er en liste over modeller leveret af Symbolic-teamet, som er blevet testet og verificeret til at fungere godt med Symbolic. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Symbolic Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg symbolic og gå til [symbolic.computer/auth](https://symbolic.computer/auth).
...
Det fungerer som alle andre udbydere i Symbolic og er helt valgfrit at bruge.

---

## Symbolic Go

Symbolic Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Symbolic-teamet, som er testet og verificeret til at fungere godt med Symbolic.

1. Kør kommandoen `/connect` i TUI, vælg `Symbolic Go`, og gå til [symbolic.computer/auth](https://symbolic.computer/zen).
...
Sådan bruger du Amazon Bedrock med Symbolic:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører symbolic:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `symbolic.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Symbolic understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at symbolic skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem symbolic-konfigurationen.
...
gemme tokenet i symbolic auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Symbolic bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Symbolic
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`symbolic.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
GitLab-godkendelse er indbygget i Symbolic. Se GitLab-providerdokumentationen ovenfor.

##### Konfiguration

Tilpas gennem `symbolic.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Symbolic:
...
Sådan bruger du Google Vertex AI med Symbolic:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører symbolic.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem symbolic, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session) plugin for automatisk at logge hver Symbolic-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Symbolic-samtale opført som en separat session.
...
Du kan konfigurere symbolic til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere symbolic til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere symbolic til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Symbolic. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/symbolic) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Symbolic:
...
3. Kopier API-nøglen til brug i Symbolic.
...
6. **Vigtigt**: Før du bruger skymodeller i Symbolic, skal du hente modelinformationen lokalt:
...
Symbolic Zen er en liste over testede og verificerede modeller leveret af Symbolic-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Symbolic Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Symbolic Zen**.

   ```txt
   /connect
   ```

3. Indtast Symbolic API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem symbolic-konfigurationen.
...
5. Du kan også tilpasse dem gennem symbolic-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Symbolic:
...
Du kan også tilpasse modeller gennem symbolic-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med symbolic. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `symbolic.json`-filen i projektkataloget:
...
`limit`-felterne lader Symbolic forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `symbolic auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek symbolic-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i symbolic-konfigurationen.
