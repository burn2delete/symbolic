...
Agentferdigheter lar Symbolic oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Symbolic søker etter disse stedene:

- Prosjektkonfigurasjon: `.symbolic/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/symbolic/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Symbolic opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.symbolic/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/symbolic/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.symbolic/skills/git-release/SKILL.md` slik:
...
Symbolic viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `symbolic.json`:
...
**For innebygde agenter** (i `symbolic.json`):
