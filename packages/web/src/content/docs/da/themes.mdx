...
Med Symbolic kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Symbolic vårt eget `symbolic`-tema.
...
Symbolic støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/symbolic/themes/*.json` eller `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.symbolic/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.symbolic/themes/*.json`
