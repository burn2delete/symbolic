...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i symbolic-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i symbolic giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem symbolic-konfigurationen.
...
symbolic leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.symbolic/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `symbolic.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger symbolic modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale symbolic-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.symbolic/modes/` for projektspecifikke tilstande eller `~/.config/symbolic/modes/` for globale tilstande:
