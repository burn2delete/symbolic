...
For at fejlfinde problemer med Symbolic, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\symbolic\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `symbolic --log-level DEBUG`.

---

## Lagring

symbolic gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/symbolic/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\symbolic`
...
Symbolic Desktop kører en lokal Symbolic-server (`symbolic-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Symbolic`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/symbolic/symbolic.jsonc` (eller `~/.config/symbolic/symbolic.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/symbolic/symbolic.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\symbolic\symbolic.jsonc`
...
Symbolic kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/symbolic/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\symbolic\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Symbolic kan genopbygge den.

1. Afslut Symbolic Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/symbolic`
- **Linux**: slet `~/.cache/symbolic` (eller kør `rm -rf ~/.cache/symbolic`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\symbolic`

3. Start Symbolic Desktop på ny.

---

### Løs problemer med serverforbindelse

Symbolic Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `symbolic.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `SYMBOLIC_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `SYMBOLIC_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Symbolic Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Symbolic.

---

### Meddelelser vises ikke

Symbolic Desktop viser kun systemvarsler når:

- varsler er aktiveret for Symbolic i dine OS-indstillinger, og
...
1. Afslut Symbolic Desktop.
2. Find og slet disse filer (de findes i Symbolic Desktop-appens datamappe):

- `symbolic.settings.dat` (skrivebordsstandardserver URL)
- `symbolic.global.dat` og `symbolic.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Symbolic:
...
For at finde ud af hvilke modeller du har adgang til, kør `symbolic models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\symbolic`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. symbolic installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\symbolic`

2. Start symbolic på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge symbolic til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
symbolic vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
