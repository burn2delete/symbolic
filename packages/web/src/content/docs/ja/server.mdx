---
title: サーバー
description: HTTP 経由で Symbolic サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`symbolic serve` コマンドは、Symbolic クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `symbolic.local` |
...
HTTP 基本認証でサーバーを保護するには、`SYMBOLIC_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `symbolic` になるか、`SYMBOLIC_SERVER_USERNAME` を設定してオーバーライドします。これは、`symbolic serve` と `symbolic web` の両方に当てはまります。
...
`symbolic` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Symbolic サーバーを使用して、プログラムで Symbolic と対話します。
:::

`symbolic serve` を実行してスタンドアロンサーバーを起動できます。すでに Symbolic TUI を実行している場合でも、`symbolic serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Symbolic [IDE](/docs/ide) プラグインによって使用されます。
