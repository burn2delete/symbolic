---
title: Proveedores
description: Usando cualquier proveedor LLM en Symbolic.
---

import config from "../../../../config.mjs"
export const console = config.console

Symbolic usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Symbolic.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Symbolic
configuración.
...
Symbolic Zen es una lista de modelos proporcionados por el equipo Symbolic que han sido
probado y verificado para funcionar bien con Symbolic. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Symbolic Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione symbolic y diríjase a [symbolic.computer/auth](https://symbolic.computer/auth).
...
Funciona como cualquier otro proveedor en Symbolic y su uso es completamente opcional.

---

## Symbolic Go

Symbolic Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Symbolic que han sido
probado y verificado para funcionar bien con Symbolic.

1. Ejecute el comando `/connect` en TUI, seleccione `Symbolic Go` y diríjase a [symbolic.computer/auth](https://symbolic.computer/zen).
...
Para usar Amazon Bedrock con Symbolic:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta symbolic:
   ...
   Para una configuración persistente o específica del proyecto, utilice `symbolic.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Symbolic no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que symbolic funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración symbolic.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `Symbolic`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación symbolic.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Symbolic utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Symbolic
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `symbolic.json`. También se recomienda desactivar el uso compartido de sesiones.
...
La autenticación de GitLab está integrada en Symbolic. Consulte la documentación del proveedor GitLab anterior.

##### Configuración

Personalizar a través de `symbolic.json`:
...
Para utilizar su suscripción GitHub Copilot con symbolic:
...
Para utilizar Google Vertex AI con Symbolic:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta symbolic.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de symbolic, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session) para registrar automáticamente cada conversación Symbolic como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Symbolic enumerada como una sesión separada.
...
Puede configurar symbolic para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar symbolic para usar modelos locales a través de LM Studio.
...
Puedes configurar symbolic para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Symbolic. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/symbolic) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Symbolic:
...
3. Copie la clave API para usarla en Symbolic.
...
6. **Importante**: Antes de usar modelos en la nube en Symbolic, debe obtener la información del modelo localmente:
...
Symbolic Zen es una lista de modelos probados y verificados proporcionada por el equipo Symbolic. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Symbolic Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Symbolic Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Symbolic API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración symbolic.
...
5. También puedes personalizarlos a través de tu configuración symbolic. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Symbolic:
...
También puede personalizar modelos a través de su configuración symbolic. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con symbolic. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `symbolic.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Symbolic comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `symbolic auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración symbolic y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración symbolic.
