...
Puede agregar herramientas externas a Symbolic utilizando el _Model Context Protocol_ o MCP. Symbolic admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Symbolic Config](https://symbolic.computer/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/symbolic`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Symbolic maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Symbolic:
...
Si el servidor requiere autenticación, Symbolic le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `symbolic mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Symbolic almacenará los tokens de forma segura en `~/.local/share/symbolic/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Symbolic, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Symbolic como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Symbolic a su cuenta Sentry.
