...
Con Symbolic puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Symbolic usa nuestro propio tema `symbolic`.
...
Symbolic viene con varios temas integrados.
...
- Quiere que Symbolic coincida con la apariencia de su terminal
...
Symbolic admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/symbolic/themes/*.json` o `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.symbolic/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.symbolic/themes/*.json`
