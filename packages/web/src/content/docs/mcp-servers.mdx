...
You can add external tools to Symbolic using the _Model Context Protocol_, or MCP. Symbolic supports both local and remote servers.
...
You can define MCP servers in your [Symbolic Config](https://symbolic.computer/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/symbolic` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Symbolic automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Symbolic will:
...
If the server requires authentication, Symbolic will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `symbolic mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Symbolic will store the tokens securely in `~/.local/share/symbolic/mcp-auth.json`.
...
Your MCPs are available as tools in Symbolic, alongside built-in tools. So you can manage them through the Symbolic config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Symbolic to your Sentry account.
