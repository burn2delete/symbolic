...
There are two types of agents in Symbolic; primary agents and subagents.
...
Symbolic comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Symbolic comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Symbolic comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `symbolic.json` config file:
...
If no temperature is specified, Symbolic uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Symbolic config and the project specific config.
...
The model ID in your Symbolic config uses the format `provider/model-id`. For example, if you're using [Symbolic Zen](/docs/zen), you would use `symbolic/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `symbolic models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/SymbolicOS/symbolic).
:::
