...
Det er to typer agenter i Symbolic; primære agenter og underagenter.
...
Symbolic kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Symbolic kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

Symbolic kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `symbolic.json`:
...
- Globalt: `~/.config/symbolic/agents/`
- Per prosjekt: `.symbolic/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Symbolic modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Symbolic-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Symbolic-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Symbolic Zen](/docs/zen), vil du bruke `symbolic/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `symbolic models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/SymbolicOS/symbolic).
:::
