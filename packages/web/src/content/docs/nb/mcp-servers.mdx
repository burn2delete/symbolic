...
Du kan legge til eksterne verktøy i Symbolic ved å bruke _Model Context Protocol_ eller MCP. Symbolic støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Symbolic Config](https://symbolic.computer/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="symbolic.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/symbolic`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Symbolic håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Symbolic:
...
Hvis serveren krever autentisering, vil Symbolic be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `symbolic mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Symbolic lagre tokens sikkert i `~/.local/share/symbolic/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i Symbolic, sammen med innebygde verktøy. Så du kan administrere dem gjennom Symbolic-konfigurasjonen som alle andre verktøy.
...
```json title="symbolic.json" {11, 14-18}
{
  "$schema": "https://symbolic.computer/config.json",
  "mcp": {
    "my-mcp": {
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Symbolic til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Symbolic-samtale oppført som en separat økt.
