---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Symbolic.
---

import config from "../../../../config.mjs"
export const console = config.console

Symbolic bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Symbolic-konfigurasjonen.
...
i `~/.local/share/symbolic/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Symbolic
konfig.
...
Symbolic Zen er en liste over modeller levert av Symbolic-teamet som har vært
testet og verifisert for å fungere godt med Symbolic. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Symbolic Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Symbolic Zen` og gå til [symbolic.computer/auth](https://symbolic.computer/zen).
...
Det fungerer som alle andre leverandører i Symbolic og er helt valgfritt å bruke.

---

## Symbolic Go

Symbolic Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Symbolic-teamet som har vært
testet og verifisert for å fungere godt med Symbolic.

1. Kjør kommandoen `/connect` i TUI, velg `Symbolic Go`, og gå til [symbolic.computer/auth](https://symbolic.computer/zen).
...
Slik bruker du Amazon Bedrock med Symbolic:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Symbolic:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `symbolic.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i symbolic støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Symbolic skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Symbolic-konfigurasjonen.
...
for å lagre token i Symbolic auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Symbolic bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Symbolic
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`symbolic.json` fil. Det anbefales også å deaktivere øktdeling.
...
GitLab-autentisering er innebygd i Symbolic. Se GitLab-leverandordokumentasjonen ovenfor.

##### Konfigurasjon

Tilpass gjennom `symbolic.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Symbolic:
...
Slik bruker du Google Vertex AI med Symbolic:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Symbolic.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Symbolic, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session) plugin for automatisk å logge hver Symbolic-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Symbolic-samtale oppført som en separat økt.
...
Du kan konfigurere Symbolic for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Symbolic for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Symbolic for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Symbolic. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/symbolic) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Symbolic:
...
3. Kopier API-nøkkelen for bruk i Symbolic.
...
6. **Viktig**: Før du bruker skymodeller i Symbolic, må du hente modellinformasjonen lokalt:
...
Symbolic Zen er en liste over testede og verifiserte modeller levert av Symbolic-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Symbolic Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Symbolic Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Symbolic API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Symbolic-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Symbolic-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Symbolic:
...
Du kan også tilpasse modeller gjennom Symbolic-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Symbolic. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `symbolic.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Symbolic forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `symbolic auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Symbolic-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Symbolic-konfigurasjonen.
