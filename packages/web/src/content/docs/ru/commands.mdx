...
Создайте `.symbolic/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию symbolic или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем symbolic [config](/docs/config):
...
- Глобальный: `~/.config/symbolic/commands/`
- Для каждого проекта: `.symbolic/commands/`
...
symbolic включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
