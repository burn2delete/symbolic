...
Symbolic Go 是一项低成本的订阅服务（**$10/月**），为您提供对流行开源编程模型的可靠访问。

:::note
Symbolic Go 目前处于测试阶段。
:::

Go 的工作方式与 Symbolic 中的其他提供商一样。您订阅 Symbolic Go 并获取 API 密钥。这是**完全可选的**，您不需要它也能使用 Symbolic。
...
:::tip
我们测试了一组精选的模型和提供商，它们与 Symbolic 配合良好。
:::
...
Symbolic Go 让您可以以 **$10/月** 的价格访问这些模型。
...
1. 登录 **<a href={console}>Symbolic Zen</a>**，订阅 Go，并复制您的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 `Symbolic Go`，然后粘贴您的 API 密钥。
3. 在 TUI 中运行 `/models` 以查看通过 Go 可用的模型列表。

:::note
每个工作区只有一名成员可以订阅 Symbolic Go。
:::
...
Symbolic Go 是一个 **$10/月** 的订阅计划。以下是**每 1M token** 的价格。
...
Symbolic 配置中的 [model id](/docs/config/#models) 使用 `symbolic-go/<model-id>` 格式。例如，对于 Kimi K2.5，您将在配置中使用 `symbolic-go/kimi-k2.5`。
...
4. **无锁定**，允许您在 Symbolic 中使用任何其他提供商。
