...
Pour déboguer les problèmes avec Symbolic, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\symbolic\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `symbolic --log-level DEBUG`.

---

## Stockage

symbolic stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/symbolic/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\symbolic`
...
Symbolic Desktop exécute un serveur Symbolic local (le side-car `symbolic-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Symbolic` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/symbolic/symbolic.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\symbolic\symbolic.jsonc`
...
Symbolic peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/symbolic/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\symbolic\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Symbolic puisse le reconstruire.

1. Quittez complètement Symbolic Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/symbolic`
- **Linux** : supprimez `~/.cache/symbolic` (ou exécutez `rm -rf ~/.cache/symbolic`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\symbolic`

3. Redémarrez le bureau Symbolic.

---

### Résoudre les problèmes de connexion au serveur

Symbolic Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `symbolic.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `SYMBOLIC_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `SYMBOLIC_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Symbolic Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Symbolic.

---

### Les notifications ne s'affichent pas

Symbolic Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Symbolic dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Symbolic.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Symbolic Desktop) :

- `symbolic.settings.dat` (serveur de bureau par défaut URL)
- `symbolic.global.dat` et `symbolic.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Symbolic :
...
3. Assurez-vous d'avoir la dernière version avec `symbolic upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `symbolic models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\symbolic`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. symbolic installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\symbolic`

2. Redémarrez symbolic pour réinstaller les derniers packages du fournisseur

Cela forcera symbolic à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
symbolic détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
