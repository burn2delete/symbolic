---
title: Configuration
description: Utilisation de la configuration JSON Symbolic.
---

Vous pouvez configurer Symbolic à l'aide d'un fichier de configuration JSON.

---

## Format

Symbolic prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="symbolic.jsonc"
{
...
1. **Configuration distante** (à partir de `.well-known/symbolic`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/symbolic/symbolic.json`) - préférences utilisateur
3. **Configuration personnalisée** (`SYMBOLIC_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`symbolic.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.symbolic`** - agents, commandes, plugins
6. **Configuration en ligne** (`SYMBOLIC_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.symbolic` et `~/.config/symbolic` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/symbolic`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/symbolic"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale Symbolic dans `~/.config/symbolic/symbolic.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/symbolic/tui.json`.
...
Ajoutez `symbolic.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Symbolic démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `SYMBOLIC_CONFIG`.

```bash
export SYMBOLIC_CONFIG=/path/to/my/custom-config.json
symbolic run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `SYMBOLIC_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.symbolic`, et devrait suivre la même structure.

```bash
export SYMBOLIC_CONFIG_DIR=/path/to/my/config-directory
symbolic run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.symbolic`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`symbolic.computer/config.json`**](https://symbolic.computer/config.json).

La configuration TUI utilise [**`symbolic.computer/tui.json`**](https://symbolic.computer/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://symbolic.computer/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Utilisez `SYMBOLIC_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `symbolic.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `symbolic serve` et `symbolic web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Symbolic.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `symbolic.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Symbolic via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Symbolic essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/symbolic/agents/` ou `.symbolic/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Symbolic reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`symbolic run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/symbolic/commands/` ou `.symbolic/commands/`. [En savoir plus ici](/docs/commands).
...
Symbolic téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, symbolic **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Symbolic avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.symbolic/plugins/` ou `~/.config/symbolic/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="symbolic.json"
{
  "$schema": "https://symbolic.computer/config.json",
  "plugin": ["symbolic-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre Symbolic à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
