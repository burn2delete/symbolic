---
title: Serveur
description: Interagissez avec le serveur symbolic via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `symbolic serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client symbolic peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `symbolic.local` |
...
Définissez `SYMBOLIC_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `symbolic`, ou définissez `SYMBOLIC_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `symbolic serve` et à `symbolic web`.
...
Lorsque vous exécutez `symbolic`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur symbolic pour interagir avec symbolic par programmation.
:::

Cette architecture permet à symbolic de prendre en charge plusieurs clients et vous permet d'interagir avec symbolic par programme.

Vous pouvez exécuter `symbolic serve` pour démarrer un serveur autonome. Si vous avez le
symbolic TUI en cours d'exécution, `symbolic serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Symbolic [IDE](/docs/ide).
...
Le serveur symbolic expose les API suivantes.
