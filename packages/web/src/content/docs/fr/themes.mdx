...
Avec Symbolic, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Symbolic utilise notre propre thème `symbolic`.
...
Symbolic est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Symbolic corresponde à l'apparence de leur terminal
...
Symbolic prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/symbolic/themes/*.json` ou `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.symbolic/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.symbolic/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/symbolic/themes
vim ~/.config/symbolic/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .symbolic/themes
vim .symbolic/themes/my-theme.json
```
