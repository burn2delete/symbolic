...
Il existe deux types d'agents dans Symbolic ; agents primaires et sous-agents.
...
Symbolic est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Symbolic est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

Symbolic est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `symbolic.json` :
...
- Global : `~/.config/symbolic/agents/`
- Par projet : `.symbolic/agents/`

```markdown title="~/.config/symbolic/agents/review.md"
---
...
Si aucune température n'est spécifiée, Symbolic utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Symbolic et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Symbolic utilise le format `provider/model-id`. Par exemple, si vous utilisez [Symbolic Zen](/docs/zen), vous utiliserez `symbolic/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="symbolic.json" {3-6,9-12}
{
  "$schema": "https://symbolic.computer/config.json",
  "tools": {
    "write": true,
...
```json title="symbolic.json" {3-5,8-10}
{
  "$schema": "https://symbolic.computer/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `symbolic models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/SymbolicOS/symbolic).
:::
...
```markdown title="~/.config/symbolic/agents/docs-writer.md"
---
...
```markdown title="~/.config/symbolic/agents/security-auditor.md"
---
