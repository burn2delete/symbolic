...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("symbolic-find")
  api.delete("symbolic-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("symbolic-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("symbolic-find", new Highlight(...rest))
    return true
  }
