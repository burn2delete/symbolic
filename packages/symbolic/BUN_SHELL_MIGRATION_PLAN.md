...
Replace runtime Bun shell template-tag usage in `packages/symbolic/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/symbolic/src` is removed except:
