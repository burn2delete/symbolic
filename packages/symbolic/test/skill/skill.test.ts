...
test("discovers skills from .symbolic/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".symbolic", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".symbolic", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".symbolic", "skill", "dir-skill")
...
test("discovers multiple skills from .symbolic/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".symbolic", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".symbolic", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".symbolic", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("discovers global skills from ~/.claude/skills/ directory", async () => {
  ...
  const originalHome = process.env.SYMBOLIC_TEST_HOME
  process.env.SYMBOLIC_TEST_HOME = tmp.path
  ...
  try {
    ...
    process.env.SYMBOLIC_TEST_HOME = originalHome
  }
})
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const symbolicSkillDir = path.join(dir, ".symbolic", "skill", "agent-skill")
      const symbolicSkillsDir = path.join(dir, ".symbolic", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(symbolicSkillDir, "SKILL.md"),
        `---
name: symbolic-skill
description: A skill in the .symbolic/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(symbolicSkillsDir, "SKILL.md"),
        `---
name: symbolic-skill
description: A skill in the .symbolic/skills directory.
---
