...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.SYMBOLIC_TEST ?? "")'), {
      env: {
        SYMBOLIC_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "SYMBOLIC_TEST_SHELL"], {
      shell: true,
      env: {
        SYMBOLIC_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("SYMBOLIC_TEST_SHELL=ok")
  })
