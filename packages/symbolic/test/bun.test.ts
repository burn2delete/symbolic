...
describe("BunProc install", () => {
  test("uses pinned cache without checking the registry", async () => {
    ...
    const raw = await fs.readFile(data, "utf8").catch(() => "")
    const json = raw ? ((JSON.parse(raw) as { dependencies?: Record<string, string> }) ?? {}) : {}
