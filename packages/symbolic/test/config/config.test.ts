...
const managedConfigDir = process.env.SYMBOLIC_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "symbolic.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "symbolic.json") {
...
test("ignores legacy tui keys in symbolic config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://symbolic.computer/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "symbolic.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["SYMBOLIC_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      symbolic: {
        options: {
          apiKey: "{env:SYMBOLIC_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["symbolic"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["SYMBOLIC_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["SYMBOLIC_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "symbolic.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .symbolic directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const symbolicDir = path.join(dir, ".symbolic")
      await fs.mkdir(symbolicDir, { recursive: true })
      const agentDir = path.join(symbolicDir, "agent")
...
test("loads agents from .symbolic/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const symbolicDir = path.join(dir, ".symbolic")
      await fs.mkdir(symbolicDir, { recursive: true })

      const agentsDir = path.join(symbolicDir, "agents")
...
test("loads commands from .symbolic/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const symbolicDir = path.join(dir, ".symbolic")
      await fs.mkdir(symbolicDir, { recursive: true })

      const commandDir = path.join(symbolicDir, "command")
...
test("loads commands from .symbolic/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const symbolicDir = path.join(dir, ".symbolic")
      await fs.mkdir(symbolicDir, { recursive: true })

      const commandsDir = path.join(symbolicDir, "commands")
...
test("does not try to install dependencies in read-only SYMBOLIC_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.SYMBOLIC_CONFIG_DIR
  process.env.SYMBOLIC_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.SYMBOLIC_CONFIG_DIR
    else process.env.SYMBOLIC_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable SYMBOLIC_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "symbolic.json"),
        JSON.stringify({ $schema: "https://symbolic.computer/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const specs = pluginEntries.map((item) => Config.pluginSpec(item))
      ...
      const scoped = specs.find((entry) => entry === expected)
      expect(scoped).toBeDefined()
      expect(scoped?.includes("/node_modules/@scope/plugin/")).toBe(true)
    },
  })
})

test("resolves scoped npm plugin tuples in config while preserving options", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const pluginDir = path.join(dir, "node_modules", "@scope", "plugin")
      await fs.mkdir(pluginDir, { recursive: true })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const config = await Config.get()
      const plugin = config.plugin?.[0]
      expect(plugin).toBeDefined()
      expect(plugin && Config.pluginSpec(plugin)).toContain("/node_modules/@scope/plugin/")
      expect(plugin && Config.pluginOptions(plugin)).toEqual({ compact: true })
    },
  })
})
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .symbolic config
      const projectDir = path.join(dir, "project")
      const symbolicDir = path.join(projectDir, ".symbolic")
      await fs.mkdir(symbolicDir, { recursive: true })
      ...
      // Local .symbolic config with different plugins
      await Filesystem.write(
        path.join(symbolicDir, "symbolic.json"),
        JSON.stringify({
          $schema: "https://symbolic.computer/config.json",
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const specs = plugins.map((item) => Config.pluginSpec(item))

      // Should contain both global and local plugins
      expect(specs.some((p) => p.includes("global-plugin-1"))).toBe(true)
      expect(specs.some((p) => p.includes("global-plugin-2"))).toBe(true)
      expect(specs.some((p) => p.includes("local-plugin-1"))).toBe(true)

      // Should have all 3 plugins (not replaced, but merged)
      const pluginNames = specs.filter((p) => p.includes("global-plugin") || p.includes("local-plugin"))
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .symbolic config with some overlapping plugins
      await Filesystem.write(
        path.join(symbolicDir, "symbolic.json"),
        JSON.stringify({
          $schema: "https://symbolic.computer/config.json",
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(specs.some((p) => p.includes("global-plugin-1"))).toBe(true)
      expect(specs.some((p) => p.includes("local-plugin-1"))).toBe(true)
      expect(specs.some((p) => p.includes("duplicate-plugin"))).toBe(true)

      // Should deduplicate the duplicate plugin
      const duplicatePlugins = specs.filter((p) => p.includes("duplicate-plugin"))
      expect(duplicatePlugins.length).toBe(1)

      // Should have exactly 3 unique plugins
      const pluginNames = specs.filter(
...
// Note: preload.ts sets SYMBOLIC_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .symbolic config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .symbolic directory config enables it
      const symbolicDir = path.join(dir, ".symbolic")
      await fs.mkdir(symbolicDir, { recursive: true })
      await Filesystem.write(
        path.join(symbolicDir, "symbolic.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/symbolic")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/symbolic")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-symbolic@2.4.3")).toBe("oh-my-symbolic")
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-symbolic@2.4.3", "file:///project/.symbolic/plugin/oh-my-symbolic.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.symbolic/plugin/oh-my-symbolic.js")
  })
  ...
  test("local plugin directory overrides global symbolic.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const symbolicDir = path.join(projectDir, ".symbolic")
        const pluginDir = path.join(symbolicDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const myPlugins = plugins.filter((p) => Config.getPluginName(Config.pluginSpec(p)) === "my-plugin")
        expect(myPlugins.length).toBe(1)
        expect(Config.pluginSpec(myPlugins[0]).startsWith("file://")).toBe(true)
      },
    })
  })
})

describe("SYMBOLIC_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"]
    process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .symbolic/ directories when flag is set", async () => {
    const originalEnv = process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"]
    process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .symbolic directory with a command
          const symbolicDir = path.join(dir, ".symbolic", "command")
          await fs.mkdir(symbolicDir, { recursive: true })
          await Filesystem.write(path.join(symbolicDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .symbolic should NOT be in directories list
          const hasProjectSymbolic = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectSymbolic).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SYMBOLIC_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["SYMBOLIC_CONFIG_DIR"]
      process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["SYMBOLIC_CONFIG_DIR"]
      } else {
        process.env["SYMBOLIC_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("SYMBOLIC_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["SYMBOLIC_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["SYMBOLIC_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["SYMBOLIC_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from SYMBOLIC_CONFIG_DIR, not project
...
describe("SYMBOLIC_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in SYMBOLIC_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SYMBOLIC_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["SYMBOLIC_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://symbolic.computer/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["SYMBOLIC_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["SYMBOLIC_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in SYMBOLIC_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["SYMBOLIC_CONFIG_CONTENT"]
