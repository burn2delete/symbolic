...
describe("ProviderTransform.message - anthropic empty content filtering", () => {
  ...
  test("filters out empty content for bedrock provider", () => {
    const bedrockModel = {
      ...anthropicModel,
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const symbolicModel = {
      ...
      api: {
        id: "symbolic-test",
        url: "https://api.symbolic.computer",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, symbolicModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.symbolic?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.symbolic?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              symbolic: { itemId: "msg_symbolic_part" },
    ...
    expect(result[0].providerOptions?.symbolic?.itemId).toBe("msg_symbolic")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.symbolic?.itemId).toBe("msg_symbolic_part")
