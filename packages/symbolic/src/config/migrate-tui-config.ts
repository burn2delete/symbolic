...
const TUI_SCHEMA_URL = "https://symbolic.computer/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from symbolic.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const symbolic = await symbolicFiles(input)
  for (const file of symbolic) {
...
async function symbolicFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.SYMBOLIC_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("symbolic", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "symbolic")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "symbolic"))
  }
  if (Flag.SYMBOLIC_CONFIG) files.push(Flag.SYMBOLIC_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "symbolic"))
