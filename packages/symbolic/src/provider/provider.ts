...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async symbolic(input) {
      const hasKey = await (async () => {
        const env = Env.all()
        if (input.env.some((item) => env[item])) return true
        if (await Auth.get(input.id)) return true
        const config = await Config.get()
        if (config.provider?.["symbolic"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from symbolic.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://symbolic.computer/",
            "X-Title": "symbolic",
          },
        },
      }
    ...
    gitlab: async (input) => {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `symbolic/${Installation.VERSION} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `symbolic auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "symbolic",
          },
        },
      }
    },
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@symbolic-agent/Provider") {}
    ...
    Effect.gen(function* () {
      const state = yield* InstanceState.make<State>(
        Effect.fn("Provider.state")(() =>
          Effect.promise(async () => {
            ...
            const models = new Map<string, LanguageModelV2>()
            const modelLoaders: Record<string, CustomModelLoader> = {}
            const varsLoaders: Record<string, CustomVarsLoader> = {}
            const discoveryLoaders: Record<string, CustomDiscoverModels> = {}
            ...
            for (const [id, provider] of Object.entries(providers)) {
              ...
              for (const [modelID, model] of Object.entries(provider.models)) {
                ...
                if (model.status === "alpha" && !Flag.SYMBOLIC_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
            ...
            if (discoveryLoaders[gitlab] && providers[gitlab]) {
              try {
                ...
                log.warn("state discovery error", { id: "gitlab", error })
              }
            }
      ...
      const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
        ...
        if (provider) {
          ...
          if (providerID.startsWith("symbolic")) {
            priority = ["gpt-5-nano"]
          }
      ...
      const defaultModel = Effect.fn("Provider.defaultModel")(function* () {
        ...
        const recent = yield* Effect.promise(() =>
          Filesystem.readJson<{ recent?: { providerID: ProviderID; modelID: ModelID }[] }>(
