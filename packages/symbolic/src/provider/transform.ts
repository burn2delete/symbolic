...
export namespace ProviderTransform {
  export const OUTPUT_TOKEN_MAX = Flag.SYMBOLIC_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
  ...
  function normalizeMessages(
    ...
    // Anthropic and Bedrock reject messages with empty content - filter out empty string
    // messages and remove empty text/reasoning parts from array content
  ...
  export function options(input: {
    ...
    if (
      ...
      (input.model.providerID === "symbolic" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
    ) {
    ...
    if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
      ...
      if (input.model.providerID.startsWith("symbolic")) {
        result["promptCacheKey"] = input.sessionID
