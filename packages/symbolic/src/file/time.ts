...
export namespace FileTime {
  ...
  export class Service extends ServiceMap.Service<Service, Interface>()("@symbolic-agent/FileTime") {}
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const assert = Effect.fn("FileTime.assert")(function* (sessionID: SessionID, filepath: string) {
        if (Flag.SYMBOLIC_DISABLE_FILETIME_CHECK) return
  ...
  export function read(sessionID: SessionID, file: string) {
    return runPromise((svc) => svc.read(sessionID, file))
  }

  export function get(sessionID: SessionID, file: string) {
    return runPromise((svc) => svc.get(sessionID, file))
  }

  export async function assert(sessionID: SessionID, filepath: string) {
    return runPromise((svc) => svc.assert(sessionID, filepath))
  }

  export async function withLock<T>(filepath: string, fn: () => Promise<T>): Promise<T> {
    return runPromise((svc) => svc.withLock(filepath, fn))
  }
}
