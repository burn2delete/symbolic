...
import type * as SDK from "@symbolic-ai/sdk/v2"
...
export namespace ShareNext {
  ...
  export async function request(): Promise<{
    ...
    if (!active?.active_org_id) {
      const baseUrl = await Config.get().then((x) => x.enterprise?.url ?? "https://symbolic.computer")
  ...
  const disabled = process.env["SYMBOLIC_DISABLE_SHARE"] === "true" || process.env["SYMBOLIC_DISABLE_SHARE"] === "1"
