...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for symbolic.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal symbolic sessions
...
symbolic acp --cwd /path/to/project
```
...
```bash
SYMBOLIC_ENABLE_QUESTION_TOOL=1 symbolic acp
```
...
- **Terminal Integration**: Full terminal support via symbolic's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | symbolic acp
```
...
ACP sessions map cleanly to symbolic's internal session model:
