...
import type { AssistantMessage, Event, SymbolicClient, SessionMessageResponse, ToolPart } from "@symbolic-agent/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: SymbolicClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `symbolic auth login` in the terminal",
        name: "Login with Symbolic",
        id: "symbolic-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Symbolic stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const symbolicProvider = providers.find((p) => p.id === "symbolic")
    if (symbolicProvider) {
      if (symbolicProvider.models["big-pickle"]) {
        return { providerID: ProviderID.symbolic, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(symbolicProvider.models))
      if (best) {
        return {
