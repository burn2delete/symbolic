...
Le skill degli agenti permettono a Symbolic di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Symbolic cerca in queste posizioni:

- Config di progetto: `.symbolic/skills/<name>/SKILL.md`
- Config globale: `~/.config/symbolic/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Symbolic risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.symbolic/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/symbolic/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.symbolic/skills/git-release/SKILL.md` cosi':
...
Symbolic elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `symbolic.json`:
...
**Per agenti integrati** (in `symbolic.json`):
