---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Symbolic.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Symbolic fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Symbolic avvia la TUI per la directory corrente.
...
Quando usi la TUI di Symbolic, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Symbolic. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Symbolic. _Alias_: `/quit`, `/q`
...
Questo è separato da `symbolic.json`, che configura il comportamento di server/runtime.
...
Usa `SYMBOLIC_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.
