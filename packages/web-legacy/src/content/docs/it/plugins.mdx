---
title: Plugin
description: Scrivi plugin per estendere Symbolic.
---

I plugin ti permettono di estendere Symbolic agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Symbolic.
...
- `.symbolic/plugins/` - plugin a livello progetto
- `~/.config/symbolic/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/symbolic/node_modules/`.
...
1. Config globale (`~/.config/symbolic/symbolic.json`)
2. Config di progetto (`symbolic.json`)
3. Directory plugin globale (`~/.config/symbolic/plugins/`)
4. Directory plugin di progetto (`.symbolic/plugins/`)
...
Symbolic esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di symbolic per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere symbolic.
...
:::note
Se usi l'app desktop di Symbolic, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a symbolic di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a symbolic:
...
L'helper `tool` crea uno strumento personalizzato che symbolic può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in symbolic insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://symbolic.computer/docs/sdk) per i dettagli.
