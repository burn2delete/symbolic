...
Symbolic Go è un abbonamento a basso costo di **$10/mese** che ti offre un accesso affidabile ai modelli di coding open source più popolari.

:::note
Symbolic Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in Symbolic. Ti abboni a Symbolic Go e ottieni la tua chiave API. È **completamente opzionale** e non è necessario utilizzarlo per usare Symbolic.
...
Symbolic Go ti dà accesso a questi modelli per **$10/mese**.

---

## Come funziona

Symbolic Go funziona come qualsiasi altro provider in Symbolic.

1. Accedi a **<a href={console}>Symbolic Zen</a>**, abbonati a Go e copia la tua chiave API.
2. Esegui il comando `/connect` nella TUI, seleziona `Symbolic Go` e incolla la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Symbolic Go.
:::
...
Symbolic Go include i seguenti limiti:
...
Symbolic Go è un piano di abbonamento da **$10/mese**. Di seguito sono riportati i prezzi **per 1M di token**.
...
Il [model id](/docs/config/#models) nella tua configurazione Symbolic utilizza il formato `symbolic-go/<model-id>`. Ad esempio, per Kimi K2.5, useresti `symbolic-go/kimi-k2.5` nella tua configurazione.
...
Abbiamo creato Symbolic Go per:
...
4. Non avere **alcun lock-in** permettendoti di utilizzare qualsiasi altro provider con Symbolic.
