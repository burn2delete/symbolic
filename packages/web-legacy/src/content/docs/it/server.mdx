---
title: Server
description: Interagisci con il server symbolic via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `symbolic serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client symbolic.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `symbolic.local` |
...
Imposta `SYMBOLIC_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `symbolic`, oppure imposta `SYMBOLIC_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `symbolic serve` sia per `symbolic web`.
...
Quando esegui `symbolic` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server symbolic per interagire con symbolic in modo programmatico.
:::

Questa architettura permette a symbolic di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `symbolic serve` per avviare un server standalone. Se la TUI di symbolic e gia in esecuzione, `symbolic serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Symbolic.
...
Il server symbolic espone le seguenti API.
