...
In Symbolic esistono due tipi di agenti: agenti primari e subagenti.
...
Symbolic include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Symbolic include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

Symbolic include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `symbolic.json`:
...
- Globale: `~/.config/symbolic/agents/`
- Per progetto: `.symbolic/agents/`
...
Se non specifichi una temperature, Symbolic usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Symbolic sia per la config specifica del progetto.
...
L'ID modello nella configurazione Symbolic usa il formato `provider/model-id`. Per esempio, se stai usando [Symbolic Zen](/docs/zen), useresti `symbolic/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `symbolic models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/SymbolicOS/symbolic).
:::
