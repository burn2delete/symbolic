---
title: Plugins
description: Skriv dine egne plugins for at udvide Symbolic.
---

Plugins giver dig mulighed for at udvide Symbolic ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Symbolics standardadfærd.
...
- `.symbolic/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/symbolic/node_modules/`.
...
1. Global konfiguration (`~/.config/symbolic/symbolic.json`)
2. Projektkonfiguration (`symbolic.json`)
3. Global plugin-mappe (`~/.config/symbolic/plugins/`)
4. Projekt plugin bibliotek (`.symbolic/plugins/`)
...
Symbolic kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En symbolic SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide symbolic.
...
:::note
Hvis du bruger Symbolic desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå symbolic i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til symbolic:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som symbolic kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for symbolic sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://symbolic.computer/docs/sdk) for detaljer.
