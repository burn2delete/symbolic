---
title: Server
description: Interager med symbolic-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `symbolic serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en symbolic-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `symbolic.local` |
...
Indstil `SYMBOLIC_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `symbolic`, eller indstil `SYMBOLIC_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `symbolic serve` og `symbolic web`.
...
Når du kører `symbolic` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug symbolic-serveren til at interagere med symbolic programmatisk.
:::

Denne arkitektur lader symbolic understøtte flere klienter og lader dig interagere med symbolic programmatisk.

Du kan køre `symbolic serve` for at starte en selvstændig server. Hvis du har
symbolic TUI kørende, vil `symbolic serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Symbolic [IDE](/docs/ide) plugins.
...
Symbolic-serveren viser følgende API'er.
