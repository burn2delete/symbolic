...
description: Skriv dine egne programtillegg for å utvide Symbolic.
---

Plugins lar deg utvide Symbolic ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Symbolic.
...
- `.symbolic/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/symbolic/node_modules/`.
...
1. Global konfigurasjon (`~/.config/symbolic/symbolic.json`)
2. Prosjektkonfigurasjon (`symbolic.json`)
3. Global plugin-katalog (`~/.config/symbolic/plugins/`)
4. Prosjektpluginkatalog (`.symbolic/plugins/`)
...
Symbolic kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Symbolic SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Symbolic.
...
:::note
Hvis du bruker Symbolic-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Symbolic fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Symbolic:
...
`tool`-hjelperen lager et tilpasset verktøy som Symbolic kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Symbolic sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://symbolic.computer/docs/sdk) for detaljer.
