---
title: Tjener
description: Samhandle med Symbolic-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `symbolic serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Symbolic-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `symbolic.local` |
...
Sett `SYMBOLIC_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `symbolic` som standard, eller sett `SYMBOLIC_SERVER_USERNAME` for å overstyre det. Dette gjelder både `symbolic serve` og `symbolic web`.
...
Når du kjører `symbolic` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Symbolic-serveren til å samhandle med Symbolic programmatisk.
:::

Denne arkitekturen lar Symbolic støtte flere klienter og lar deg samhandle med Symbolic programmatisk.

Du kan kjøre `symbolic serve` for å starte en frittstående server. Hvis du har
Symbolic TUI kjører, vil `symbolic serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Symbolic [IDE](/docs/ide) plugins.
...
symbolic-serveren viser følgende APIer.
