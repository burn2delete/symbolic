---
title: Konfigurasjon
description: Ved å bruke symbolic JSON-konfigurasjonen.
---

Du kan konfigurere Symbolic ved å bruke en JSON konfigurasjonsfil.

---

## Format

Symbolic støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "symbolic"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/symbolic`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/symbolic/symbolic.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`SYMBOLIC_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`symbolic.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.symbolic` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`SYMBOLIC_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.symbolic` og `~/.config/symbolic` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/symbolic`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Symbolic-konfigurasjon i `~/.config/symbolic/symbolic.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `symbolic.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Symbolic starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `SYMBOLIC_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `SYMBOLIC_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.symbolic` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.symbolic`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`symbolic.computer/config.json`**](https://symbolic.computer/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `symbolic serve` og `symbolic web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Symbolic-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `symbolic.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Symbolic-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Symbolic å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/symbolic/agents/` eller `.symbolic/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Symbolic falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`symbolic run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/symbolic/commands/` eller `.symbolic/commands/`. [Les mer her](/docs/commands).
...
Symbolic vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Symbolic alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Symbolic med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.symbolic/plugins/` eller `~/.config/symbolic/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Symbolic til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
