...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i Symbolic-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i Symbolic lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom Symbolic-konfigurasjonen.
...
Symbolic kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.symbolic/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `symbolic.json` konfigurasjonsfil:
...
- Globalt: `~/.config/symbolic/modes/`
- Prosjekt: `.symbolic/modes/`
...
Hvis ingen temperatur er spesifisert, bruker Symbolic modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Symbolic-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.symbolic/modes/` for prosjektspesifikke moduser eller `~/.config/symbolic/modes/` for globale moduser:
