---
title: Plugins
description: Escreva seus próprios plugins para estender o symbolic.
---

Plugins permitem que você estenda o symbolic conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do symbolic.
...
- `.symbolic/plugins/` - Plugins em nível de projeto
- `~/.config/symbolic/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/symbolic/node_modules/`.
...
1. Configuração global (`~/.config/symbolic/symbolic.json`)
2. Configuração do projeto (`symbolic.json`)
3. Diretório de plugins global (`~/.config/symbolic/plugins/`)
4. Diretório de plugins do projeto (`.symbolic/plugins/`)
...
O symbolic executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do symbolic para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o symbolic.
...
:::note
Se você estiver usando o aplicativo desktop symbolic, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o symbolic de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao symbolic:
...
O helper `tool` cria uma ferramenta personalizada que o symbolic pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o symbolic junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://symbolic.computer/docs/sdk) para detalhes.
