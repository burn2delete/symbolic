...
description: Crie ferramentas que o LLM pode chamar no symbolic.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do symbolic, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.symbolic/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/symbolic/tools/`.
