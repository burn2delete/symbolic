...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do symbolic. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no symbolic permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do symbolic.
...
O symbolic vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.symbolic/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `symbolic.json`:
...
- Global: `~/.config/symbolic/modes/`
- Projeto: `.symbolic/modes/`

```markdown title="~/.config/symbolic/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o symbolic usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do symbolic quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.symbolic/modes/` para modos específicos do projeto ou `~/.config/symbolic/modes/` para modos globais:

```markdown title=".symbolic/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/symbolic/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
