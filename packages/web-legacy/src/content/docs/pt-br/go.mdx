...
O Symbolic Go é uma assinatura de baixo custo de **$10/mês** que oferece acesso confiável a modelos de codificação abertos populares.

:::note
O Symbolic Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no Symbolic. Você assina o Symbolic Go e obtém sua chave de API. É **totalmente opcional** e você não precisa usá-lo para usar o Symbolic.
...
O Symbolic Go oferece acesso a esses modelos por **$10/mês**.

---

## Como funciona

O Symbolic Go funciona como qualquer outro provedor no Symbolic.

1. Você faz login no **<a href={console}>Symbolic Zen</a>**, assina o Go e copia sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Symbolic Go` e cola sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Symbolic Go.
:::
...
O Symbolic Go inclui os seguintes limites:
...
O Symbolic Go é um plano de assinatura de **$10/mês**. Abaixo estão os preços **por 1M de tokens**.
...
O [model id](/docs/config/#models) (ID do modelo) na sua configuração do Symbolic usa o formato `symbolic-go/<model-id>`. Por exemplo, para o Kimi K2.5, você usaria `symbolic-go/kimi-k2.5` na sua configuração.
...
4. Não ter **nenhum bloqueio (lock-in)**, permitindo que você use qualquer outro provedor com o Symbolic também.
