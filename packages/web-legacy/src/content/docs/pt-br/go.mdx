...
O Symbolic Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

:::note
O Symbolic Go está atualmente em beta.
:::

O Go funciona como qualquer outro provedor no Symbolic. Você assina o Symbolic Go e
...
O Symbolic Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.
...
1. Você entra no **<a href={console}>Symbolic Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Symbolic Go` e cola
   a sua chave de API.
...
O [ID do modelo](/docs/config/#models) na sua configuração do Symbolic
usa o formato `symbolic-go/<model-id>`. Por exemplo, para o Kimi K2.5, você usaria
`symbolic-go/kimi-k2.5` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Symbolic também.
