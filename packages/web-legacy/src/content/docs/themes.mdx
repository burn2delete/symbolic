...
With Symbolic you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Symbolic uses our own `symbolic` theme.
...
Symbolic comes with several built-in themes.
...
- Want Symbolic to match their terminal's appearance
...
Symbolic supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/symbolic/themes/*.json` or `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.symbolic/themes/*.json`
4. **Current working directory** - Defined in `./.symbolic/themes/*.json`
