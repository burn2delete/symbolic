---
title: SDK
description: Client JS de type sécurisé pour le serveur symbolic.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK symbolic JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler symbolic par programme.
...
```bash
npm install @symbolic-agent/sdk
```
...
Créez une instance de symbolic :

```javascript
import { createSymbolic } from "@symbolic-agent/sdk"

const { client } = await createSymbolic()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `symbolic.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${symbolic.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de symbolic, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@symbolic-agent/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*symbolic" },
})
