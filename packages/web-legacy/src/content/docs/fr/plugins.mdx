---
title: Plugins
description: Écrivez vos propres plugins pour étendre Symbolic.
---

Les plugins vous permettent d'étendre Symbolic en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Symbolic.
...
- `.symbolic/plugins/` - Plugins au niveau du projet
- `~/.config/symbolic/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/symbolic/node_modules/`.
...
1. Configuration globale (`~/.config/symbolic/symbolic.json`)
2. Configuration du projet (`symbolic.json`)
3. Répertoire global des plugins (`~/.config/symbolic/plugins/`)
4. Répertoire des plugins du projet (`.symbolic/plugins/`)
...
Symbolic exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK symbolic pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre symbolic.
...
:::note
Si vous utilisez l'application de bureau Symbolic, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez symbolic de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à symbolic :
...
L'assistant `tool` crée un outil personnalisé que symbolic peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour symbolic aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://symbolic.computer/docs/sdk) pour plus de détails.
