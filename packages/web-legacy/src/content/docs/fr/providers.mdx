---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en Symbolic.
---

import config from "../../../../config.mjs"
export const console = config.console

Symbolic utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration Symbolic.
...
en `~/.local/share/symbolic/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre Symbolic
configuration.
...
Symbolic Zen est une liste de modèles fournis par l'équipe Symbolic qui ont été
testé et vérifié pour fonctionner correctement avec Symbolic. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par Symbolic Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez symbolic et dirigez-vous vers [symbolic.computer/auth](https://symbolic.computer/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans Symbolic et son utilisation est totalement facultative.

---

## Symbolic Go

Symbolic Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe Symbolic qui ont été
testé et vérifié pour fonctionner correctement avec Symbolic.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `Symbolic Go` et rendez-vous sur [symbolic.computer/auth](https://symbolic.computer/zen).
...
Pour utiliser Amazon Bedrock avec Symbolic :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de symbolic :

```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY symbolic

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile symbolic

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX symbolic
```
...
Pour une configuration spécifique au projet ou persistante, utilisez `symbolic.json` :

```json title="symbolic.json"
{
  "$schema": "https://symbolic.computer/config.json",
  "provider": {
    "amazon-bedrock": {
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans Symbolic n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que symbolic fonctionne correctement.
   :::
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX symbolic
   ```
...
5. Définissez le nom de votre ressource comme variable d'environnement :
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX symbolic
   ```
...
Vous pouvez également ajouter des modèles via votre configuration symbolic.
...
pour stocker le jeton dans le stockage d'authentification symbolic.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
Symbolic utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller Symbolic
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `symbolic.json`. Il est également recommandé de désactiver le partage de session.
...
L'authentification GitLab est intégrée à Symbolic. Voir la documentation du fournisseur GitLab ci-dessus.

##### Configuration

Personnalisez via `symbolic.json` :
...
{
  "$schema": "https://symbolic.computer/config.json",
  "plugin": ["@gitlab/symbolic-gitlab-plugin"]
}
```
...
Pour utiliser votre abonnement GitHub Copilot avec symbolic :
...
Pour utiliser Google Vertex AI avec Symbolic :
...
Définissez-les lors de l'exécution de symbolic.

```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id symbolic
```
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via symbolic, vous pouvez toujours le configurer vous-même.
...
```jsonc title="~/.config/symbolic/symbolic.jsonc"
{
  "$schema": "https://symbolic.computer/config.json",
  "provider": {
    "helicone": {
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session) pour enregistrer automatiquement chaque conversation Symbolic en tant que session dans Helicone.

```bash
npm install -g symbolic-helicone-session
```
...
```json title="symbolic.json"
{
  "plugin": ["symbolic-helicone-session"]
}
```

Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation Symbolic répertoriée comme une session distincte.
...
Vous pouvez configurer symbolic pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)

```json title="symbolic.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://symbolic.computer/config.json",
  "provider": {
    "llama.cpp": {
...
Vous pouvez configurer symbolic pour utiliser des modèles locaux via LM Studio.

```json title="symbolic.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://symbolic.computer/config.json",
  "provider": {
    "lmstudio": {
...
Vous pouvez configurer symbolic pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour Symbolic. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/symbolic) pour plus de détails.
:::

```json title="symbolic.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://symbolic.computer/config.json",
  "provider": {
    "ollama": {
...
Pour utiliser Ollama Cloud avec Symbolic :
...
3. Copiez la clé API à utiliser dans Symbolic.
...
6. **Important** : Avant d'utiliser des modèles cloud dans Symbolic, vous devez extraire les informations du modèle localement :
...
Symbolic Zen est une liste de modèles testés et vérifiés fournie par l'équipe Symbolic. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>Symbolic Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **Symbolic Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé Symbolic API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration symbolic.
...
5. Vous pouvez également les personnaliser via votre configuration symbolic. Voici un exemple de spécification d'un fournisseur
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' symbolic
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group symbolic
   ```
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec Symbolic :
...
Vous pouvez également personnaliser les modèles via votre configuration symbolic. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec symbolic. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
3. Entrez votre clé API pour le fournisseur.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in symbolic.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Créez ou mettez à jour votre fichier `symbolic.json` dans le répertoire de votre projet :

   ```json title="symbolic.json" ""myprovider"" {5-15}
   {
     "$schema": "https://symbolic.computer/config.json",
     "provider": {
       "myprovider": {
...
```json title="symbolic.json" {9,11,17-20}
{
  "$schema": "https://symbolic.computer/config.json",
  "provider": {
    "myprovider": {
...
Les champs `limit` permettent à Symbolic de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `symbolic auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration symbolic et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration symbolic.
