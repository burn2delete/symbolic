---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Symbolic.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Symbolic fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Symbolic démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Symbolic TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Symbolic. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `symbolic.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `SYMBOLIC_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.
