---
title: Providers
description: Using any LLM provider in Symbolic.
---

import config from "../../../config.mjs"
export const console = config.console

Symbolic uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Symbolic config.
...
You can customize the providers through the `provider` section in your Symbolic
config.
...
Symbolic Zen is a list of models provided by the Symbolic team that have been
tested and verified to work well with Symbolic. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Symbolic Zen.
:::

1. Run the `/connect` command in the TUI, select `Symbolic Zen`, and head to [symbolic.computer/auth](https://symbolic.computer/zen).
...
It works like any other provider in Symbolic and is completely optional to use.

---

## Symbolic Go

Symbolic Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Symbolic team that have been
tested and verified to work well with Symbolic.

1. Run the `/connect` command in the TUI, select `Symbolic Go`, and head to [symbolic.computer/auth](https://symbolic.computer/zen).
...
To use Amazon Bedrock with Symbolic:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running symbolic:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `symbolic.json`:
...
:::info
Using your Claude Pro/Max subscription in Symbolic is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for symbolic to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your symbolic config.
...
Symbolic uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Symbolic
to only use your own GitLab-hosted instance, add the following to your
`symbolic.json` file. It is also recommended to disable session sharing.
...
GitLab auth is built into Symbolic. See the GitLab provider docs above.
...
To use your GitHub Copilot subscription with symbolic:
...
To use Google Vertex AI with Symbolic:
...
2. Set the required environment variables:
   ...
   Set them while running symbolic.
...
In the event you see a feature or model from Helicone that isn't configured automatically through symbolic, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session) plugin to automatically log each Symbolic conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Symbolic conversation listed as a separate session.
...
You can configure symbolic to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure symbolic to use local models through LM Studio.
...
You can configure symbolic to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Symbolic. See the [Ollama integration docs](https://docs.ollama.com/integrations/symbolic) for details.
:::
...
To use Ollama Cloud with Symbolic:
...
3. Copy the API key for use in Symbolic.
...
6. **Important**: Before using cloud models in Symbolic, you must pull the model information locally:
...
Symbolic Zen is a list of tested and verified models provided by the Symbolic team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Symbolic Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Symbolic Zen**.

   ```txt
   /connect
   ```

3. Enter your Symbolic API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your symbolic config.
...
5. You can also customize them through your symbolic config. Here's an example of specifying a provider
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Symbolic:
...
You can also customize models through your symbolic config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with symbolic. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `symbolic.json` file in your project directory:
...
The `limit` fields allow Symbolic to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `symbolic auth list` to see if the credentials
...
2. For custom providers, check the symbolic config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your symbolic config.
