---
title: Server
description: Interact with symbolic server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `symbolic serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an symbolic client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `symbolic.local` |
...
Set `SYMBOLIC_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `symbolic`, or set `SYMBOLIC_SERVER_USERNAME` to override it. This applies to both `symbolic serve` and `symbolic web`.
...
When you run `symbolic` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the symbolic server to interact with symbolic programmatically.
:::

This architecture lets symbolic support multiple clients and allows you to interact with symbolic programmatically.

You can run `symbolic serve` to start a standalone server. If you have the
symbolic TUI running, `symbolic serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Symbolic [IDE](/docs/ide) plugins.
...
The symbolic server exposes the following APIs.
