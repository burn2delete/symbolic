...
Agent skills omogucavaju Symbolic da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Symbolic pretrazuje ove lokacije:

- Konfiguracija projekta: `.symbolic/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/symbolic/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Symbolic ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.symbolic/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/symbolic/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.symbolic/skills/git-release/SKILL.md` ovako:
...
Symbolic navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `symbolic.json`:
...
**Za ugradene agente** (u `symbolic.json`):
