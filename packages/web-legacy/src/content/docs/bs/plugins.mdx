---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Symbolic.
---

Dodaci vam omogućavaju da proširite Symbolic spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Symbolic.
...
- `.symbolic/plugins/` - Dodaci na nivou projekta
- `~/.config/symbolic/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/symbolic/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/symbolic/symbolic.json`)
2. Konfiguracija projekta (`symbolic.json`)
3. Globalni direktorij dodataka (`~/.config/symbolic/plugins/`)
4. Direktorij dodataka projekta (`.symbolic/plugins/`)
...
Symbolic pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Symbolic SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Symbolic.
...
Spriječite symbolic da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u symbolic:
...
Pomoćnik `tool` kreira prilagođeni alat koji symbolic može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za symbolic zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://symbolic.computer/docs/sdk) za detalje.
