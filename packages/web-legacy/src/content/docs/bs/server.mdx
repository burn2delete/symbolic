---
title: Server
description: Komunicirajte s symbolic serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `symbolic serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji symbolic klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `symbolic.local` |
...
Postavite `SYMBOLIC_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `symbolic`, ili postavite `SYMBOLIC_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `symbolic serve` i za `symbolic web`.
...
Kada pokrenete `symbolic`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite symbolic server za programsku interakciju sa symbolic-om.
:::

Ova arhitektura omogucava symbolic podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `symbolic serve` da startate standalone server. Ako je symbolic TUI vec pokrenut, `symbolic serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Symbolic [IDE](/docs/ide) pluginovi.
...
symbolic server izlaže sljedece API-je.
