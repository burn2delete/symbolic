...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Symbolic dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Symbolic.
...
:::note
Ovaj alat je dostupan samo kada je `SYMBOLIC_EXPERIMENTAL_LSP_TOOL=true` (ili `SYMBOLIC_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Symbolic provajdera ili kada je varijabla `SYMBOLIC_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Symbolic:
