...
Da biste otklonili probleme s Symbolic, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\symbolic\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `symbolic --log-level DEBUG`.

---

## Pohrana

Symbolic pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/symbolic/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\symbolic`
...
Symbolic Desktop pokreće lokalni Symbolic server (`symbolic-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Symbolic` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/symbolic/symbolic.jsonc` (ili `~/.config/symbolic/symbolic.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/symbolic/symbolic.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\symbolic\symbolic.jsonc`
...
Symbolic također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/symbolic/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\symbolic\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Symbolic mogao ponovo izgraditi.

1. Potpuno zatvorite Symbolic Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/symbolic`
- **Linux**: obrišite `~/.cache/symbolic` (ili pokrenite `rm -rf ~/.cache/symbolic`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\symbolic`

3. Ponovo pokrenite Symbolic Desktop.

---

### Rješavanje problema sa vezom na serveru

Symbolic Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `symbolic.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `SYMBOLIC_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `SYMBOLIC_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Symbolic Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Symbolic karakteristikama.

---

### Obavještenja se ne prikazuju

Symbolic Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Symbolic u postavkama vašeg OS-a, i
...
1. Zatvorite Symbolic Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Symbolic Desktop aplikacije):

- `symbolic.settings.dat` (URL zadanog servera za desktop)
- `symbolic.global.dat` i `symbolic.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `symbolic models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\symbolic`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Symbolic dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\symbolic`

2. Ponovo pokrenite Symbolic da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Symbolic da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Symbolic će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
