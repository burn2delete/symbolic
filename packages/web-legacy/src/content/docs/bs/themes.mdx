...
U Symbolic mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Symbolic koristi nasu `symbolic` temu.
...
Symbolic dolazi sa vise ugradenih tema.
...
- Zele da Symbolic odgovara izgledu njihovog terminala
...
Symbolic podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/symbolic/themes/*.json` ili `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.symbolic/themes/*.json`
4. **Trenutni radni direktorij** - `./.symbolic/themes/*.json`
