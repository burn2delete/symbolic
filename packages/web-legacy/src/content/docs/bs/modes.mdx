...
ove ili konfigurirajte svoje putem symbolic config.
...
symbolic dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.symbolic/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `symbolic.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/symbolic/modes/`
- Projekat: `.symbolic/modes/`
...
Ako temperatura nije navedena, symbolic koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.symbolic/modes/` za specifične načine rada ili `~/.config/symbolic/modes/` za globalne načine:
