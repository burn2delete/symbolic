...
description: Cree herramientas que LLM pueda llamar en symbolic.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de symbolic como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.symbolic/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/symbolic/tools/`.
...
```ts title=".symbolic/tools/database.ts" {1}
import { tool } from "@symbolic-agent/plugin"
...
```ts title=".symbolic/tools/math.ts"
import { tool } from "@symbolic-agent/plugin"
...
```ts title=".symbolic/tools/bash.ts"
import { tool } from "@symbolic-agent/plugin"
...
```ts title=".symbolic/tools/project.ts" {8}
import { tool } from "@symbolic-agent/plugin"
...
```python title=".symbolic/tools/add.py"
import sys
...
```ts title=".symbolic/tools/python-add.ts" {10}
import { tool } from "@symbolic-agent/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".symbolic/tools/add.py")
