...
Las habilidades del agente permiten a Symbolic descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Symbolic busca estas ubicaciones:

- Configuración del proyecto: `.symbolic/skills/<name>/SKILL.md`
- Configuración global: `~/.config/symbolic/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Symbolic sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.symbolic/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/symbolic/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.symbolic/skills/git-release/SKILL.md` así:
...
Symbolic enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `symbolic.json`:
...
**Para agentes integrados** (en `symbolic.json`):
