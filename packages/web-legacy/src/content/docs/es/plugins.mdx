---
title: Complementos
description: Escriba sus propios complementos para extender Symbolic.
---

Los complementos le permiten extender Symbolic al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Symbolic.
...
- `.symbolic/plugins/` - Complementos a nivel de proyecto
- `~/.config/symbolic/plugins/` - Complementos globales
...
```json title="symbolic.json"
{
  ...
  "plugin": ["symbolic-helicone-session", "symbolic-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/symbolic/node_modules/`.
...
1. Configuración global (`~/.config/symbolic/symbolic.json`)
2. Configuración del proyecto (`symbolic.json`)
3. Directorio global de complementos (`~/.config/symbolic/plugins/`)
4. Directorio de complementos del proyecto (`.symbolic/plugins/`)
...
```json title=".symbolic/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Symbolic ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".symbolic/plugins/my-plugin.ts"
...
```js title=".symbolic/plugins/example.js"
...
- `client`: Un cliente SDK symbolic para interactuar con la IA.
...
import type { Plugin } from "@symbolic-agent/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar symbolic.
...
```js title=".symbolic/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "symbolic"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Symbolic, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que symbolic lea archivos `.env`:

```javascript title=".symbolic/plugins/env-protection.js"
...
```javascript title=".symbolic/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a symbolic:

```ts title=".symbolic/plugins/custom-tools.ts"
import { type Plugin, tool } from "@symbolic-agent/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que symbolic puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para symbolic junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://symbolic.computer/docs/sdk) para obtener más detalles.
...
```ts title=".symbolic/plugins/compaction.ts"
import type { Plugin } from "@symbolic-agent/plugin"
...
```ts title=".symbolic/plugins/custom-compaction.ts"
import type { Plugin } from "@symbolic-agent/plugin"
