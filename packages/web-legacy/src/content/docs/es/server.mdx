---
title: Servidor
description: Interactuar con el servidor symbolic a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `symbolic serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente symbolic puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `symbolic.local` |
...
Configure `SYMBOLIC_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `symbolic`, o configure `SYMBOLIC_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `symbolic serve` como a `symbolic web`.
...
Cuando ejecuta `symbolic`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor symbolic para interactuar con symbolic mediante programación.
:::

Esta arquitectura permite que symbolic admita múltiples clientes y le permite interactuar con symbolic mediante programación.

Puede ejecutar `symbolic serve` para iniciar un servidor independiente. Si tienes el
symbolic TUI ejecutándose, `symbolic serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Symbolic [IDE](/docs/ide).
...
El servidor symbolic expone las siguientes API.
