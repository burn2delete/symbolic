...
Las herramientas permiten que LLM realice acciones en su código base. Symbolic viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Symbolic.
...
:::note
Esta herramienta solo está disponible cuando `SYMBOLIC_EXPERIMENTAL_LSP_TOOL=true` (o `SYMBOLIC_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Symbolic o cuando la variable de entorno `SYMBOLIC_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Symbolic:

```bash
SYMBOLIC_ENABLE_EXA=1 symbolic
```
