...
Symbolic Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
Symbolic Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en Symbolic. Te suscribes a Symbolic Go y
...
Symbolic Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.
...
1. Inicias sesión en **<a href={console}>Symbolic Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Symbolic Go` y pegas
   tu API key.
...
El [ID del modelo](/docs/config/#models) en tu configuración de Symbolic
usa el formato `symbolic-go/<model-id>`. Por ejemplo, para Kimi K2.5, usarías
`symbolic-go/kimi-k2.5` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Symbolic.
