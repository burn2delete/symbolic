...
Symbolic Go es una suscripción de bajo coste de **10 $/mes** que te ofrece acceso fiable a modelos populares de código abierto.

:::note
Symbolic Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en Symbolic. Te suscribes a Symbolic Go y obtienes tu clave API. Es **completamente opcional** y no necesitas usarlo para utilizar Symbolic.
...
:::tip
Hemos probado un grupo selecto de modelos y proveedores que funcionan bien con Symbolic.
:::
...
Symbolic Go te da acceso a estos modelos por **10 $/mes**.

---

## Cómo funciona

Symbolic Go funciona como cualquier otro proveedor en Symbolic.

1. Inicias sesión en **<a href={console}>Symbolic Zen</a>**, te suscribes a Go y copias tu clave API.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Symbolic Go` y pegas tu clave API.
3. Ejecuta `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Symbolic Go.
:::
...
Symbolic Go incluye los siguientes límites:
...
Symbolic Go es un plan de suscripción de **10 $/mes**. A continuación se muestran los precios **por 1M de tokens**.
...
El [model id](/docs/config/#models) en tu configuración de Symbolic usa el formato `symbolic-go/<model-id>`. Por ejemplo, para Kimi K2.5, usarías `symbolic-go/kimi-k2.5` en tu configuración.
...
4. **Sin ataduras**, permitiéndote usar cualquier otro proveedor con Symbolic también.
