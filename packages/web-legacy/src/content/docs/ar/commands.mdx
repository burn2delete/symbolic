...
أنشئ الملف `.symbolic/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Symbolic أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Symbolic:
...
- على مستوى النظام: `~/.config/symbolic/commands/`
- لكل مشروع: `.symbolic/commands/`
...
يتضمن symbolic عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
