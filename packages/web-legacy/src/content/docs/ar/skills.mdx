...
تتيح مهارات الوكيل لـ Symbolic اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Symbolic في هذه المواقع:

- إعدادات المشروع: `.symbolic/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/symbolic/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Symbolic صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.symbolic/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/symbolic/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.symbolic/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Symbolic المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `symbolic.json`:
...
**للوكلاء المدمجين** (في `symbolic.json`):
