...
يمكنك إضافة أدوات خارجية إلى Symbolic باستخدام _Model Context Protocol_، أو MCP. يدعم Symbolic خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Symbolic](https://symbolic.computer/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/symbolic` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Symbolic تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Symbolic بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Symbolic المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `symbolic mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Symbolic الرموز بشكل آمن في `~/.local/share/symbolic/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Symbolic إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Symbolic مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Symbolic بحساب Sentry الخاص بك.
