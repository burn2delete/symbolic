---
title: TUI
description: Using the Symbolic terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Symbolic provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Symbolic starts the TUI for the current directory.
...
When using the Symbolic TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Symbolic. Allows you to select from available providers and add their API keys.
...
Exit Symbolic. _Aliases_: `/quit`, `/q`
...
This is separate from `symbolic.json`, which configures server/runtime behavior.
...
Use `SYMBOLIC_TUI_CONFIG` to load a custom TUI config path.
