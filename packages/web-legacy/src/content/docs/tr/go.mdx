...
Symbolic Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
Symbolic Go şu anda beta olarak mevcut.
:::

Go, Symbolic'daki diğer sağlayıcılar gibi çalışır. Symbolic Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Symbolic'u kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
Symbolic ile iyi çalışan seçili bir model ve sağlayıcı grubunu test ettik.
:::
...
Symbolic Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Symbolic Go, Symbolic'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Symbolic Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Symbolic Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Symbolic Go'ya abone olabilir.
:::
...
Symbolic Go aşağıdaki limitleri içerir:
...
Symbolic yapılandırmanızdaki [model id](/docs/config/#models) formatı `symbolic-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `symbolic-go/kimi-k2.5` kullanmalısınız.
...
Symbolic Go'yu şu amaçlarla oluşturduk:
...
4. Symbolic ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
