---
title: Zen
description: symbolic tarafından sağlanan seçilmiş modeller listesi.
---
...
Symbolic Zen, symbolic ekibi tarafından test edilip doğrulanmış modellerin bir listesidir.

:::note
Symbolic Zen şu anda beta aşamasındadır.
:::

Zen, symbolic'daki diğer sağlayıcılar gibi çalışır. Symbolic Zen'e giriş yapar ve API anahtarınızı alırsınız. Tamamen isteğe bağlıdır ve symbolic kullanmak için bunu kullanmanıza gerek yoktur.
...
:::tip
symbolic ile iyi çalışan seçkin bir grup model ve sağlayıcıyı test ettik.
:::
...
Symbolic Zen, bu modellere erişmenizi sağlayan bir AI ağ geçididir.

---

## Nasıl çalışır

Symbolic Zen, symbolic'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Symbolic Zen</a>**'de oturum açın, fatura ayrıntılarınızı ekleyin ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, Symbolic Zen'i seçin ve API anahtarınızı yapıştırın.
...
symbolic yapılandırmanızdaki [model kimliği](/docs/config/#models) `symbolic/<model-id>` biçimini kullanır. Örneğin, GPT 5.2 Codex için yapılandırmanızda `symbolic/gpt-5.2-codex` kullanırsınız.
...
- MiniMax M2.5 Free, sınırlı bir süre için Symbolic'da ücretsizdir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
- Big Pickle, sınırlı bir süre için Symbolic'da ücretsiz olan gizli bir modeldir. Ekip bu süreyi geri bildirim toplamak ve modeli iyileştirmek için kullanıyor.
...
Symbolic Zen'i şu amaçlarla oluşturduk:
...
4. Başka bir kodlama ajanıyla kullanmanıza izin vererek **kilitlenmeyi önlemek**. Ve her zaman Symbolic ile başka bir sağlayıcıyı kullanmanıza izin vermek.
