---
title: Eklentiler
description: Symbolic'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Symbolic'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Symbolic'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.symbolic/plugins/` - Proje düzeyinde eklentiler
- `~/.config/symbolic/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/symbolic/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/symbolic/symbolic.json`)
2. Project config (`symbolic.json`)
3. Global eklenti dizini (`~/.config/symbolic/plugins/`)
4. Proje eklenti dizini (`.symbolic/plugins/`)
...
Symbolic bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Symbolic SDK istemcisi.
...
Symbolic'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Symbolic masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Symbolic'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Symbolic'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Symbolic'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Symbolic'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://symbolic.computer/docs/sdk) bakın.
