...
:::caution
Modlar artık symbolic yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

symbolic'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya symbolic yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
symbolic iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.symbolic/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`symbolic.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, symbolic modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global symbolic yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.symbolic/modes/` veya genel modlar için `~/.config/symbolic/modes/`'de mod dosyaları oluşturun:
