---
title: Sağlayıcılar
description: symbolic'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

symbolic, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı symbolic yapılandırmanızda yapılandırın.
...
`~/.local/share/symbolic/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları symbolic'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Symbolic Zen, symbolic ekibi tarafından sağlanan modellerin bir listesidir.
symbolic ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Symbolic Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, symbolic'u seçin ve [symbolic.computer/auth](https://symbolic.computer/auth)'ye gidin.
...
symbolic'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Symbolic Go

Symbolic Go, symbolic ile iyi çalıştığı test edilmiş ve doğrulanmış, symbolic ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Symbolic Go`'yu seçin ve [symbolic.computer/auth](https://symbolic.computer/zen) adresine gidin.
...
Amazon Bedrock'u symbolic ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   symbolic'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `symbolic.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi symbolic'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   symbolic'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Symbolic yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
to store token in symbolic auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
symbolic, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. symbolic'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`symbolic.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
GitLab kimlik doğrulaması Symbolic içinde yerleşiktir. Yukarıdaki GitLab sağlayıcı belgelerine bakın.

##### Yapılandırma

`symbolic.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi symbolic'la kullanmak için:
...
Google Vertex AI'yi symbolic ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları symbolic çalıştırılırken ayarlayın.
...
Helicone'un symbolic aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her symbolic görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [symbolic-helicone-session](https://github.com/H2Shami/symbolic-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her symbolic görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
symbolic'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla symbolic'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla symbolic'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini symbolic için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/symbolic)'a bakın.
:::
...
Ollama Cloud'u symbolic ile kullanmak için:
...
3. symbolic'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: symbolic'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Symbolic Zen, symbolic ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Symbolic Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Symbolic Zen**'i arayın.

   ```txt
   /connect
   ```

3. symbolic API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   symbolic yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca symbolic yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Symbolic ile kullanmak için:
...
Modelleri symbolic yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı symbolic ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `symbolic.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları symbolic'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `symbolic auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için symbolic yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, symbolic yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
