---
title: プラグイン
description: Symbolic を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Symbolic を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Symbolic のデフォルトの動作を変更したりできます。
...
- `.symbolic/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/symbolic/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/symbolic/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/symbolic/symbolic.json`)
2. プロジェクト設定 (`symbolic.json`)
3. グローバルプラグインディレクトリ (`~/.config/symbolic/plugins/`)
4. プロジェクトプラグインディレクトリ (`.symbolic/plugins/`)
...
Symbolic は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Symbolic SDK クライアント。
...
ここでは、Symbolic を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://symbolic.computer/docs/sdk) を参照してください。
...
Symbolic が `.env` ファイルを読み取らないようにします。
...
プラグインは Symbolic にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Symbolic が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Symbolic で使用できます。
