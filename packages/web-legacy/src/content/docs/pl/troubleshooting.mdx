...
Aby debugować problemy z symbolic, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\symbolic\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `symbolic --log-level DEBUG`.

---

## Przechowywanie danych

symbolic przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/symbolic/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\symbolic`
...
symbolic Desktop uruchamia w tle lokalny serwer symbolic (sidecar `symbolic-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Symbolic` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/symbolic/symbolic.jsonc` (lub `~/.config/symbolic/symbolic.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/symbolic/symbolic.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\symbolic\symbolic.jsonc`
...
symbolic może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/symbolic/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\symbolic\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby symbolic mógł ją odbudować.

1. Całkowicie zamknij symbolic Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/symbolic`
- **Linux**: usuń `~/.cache/symbolic` (lub uruchom `rm -rf ~/.cache/symbolic`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\symbolic`

3. Uruchom ponownie symbolic Desktop.

---

### Napraw problemy z połączeniem z serwerem

symbolic Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `symbolic.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `SYMBOLIC_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `SYMBOLIC_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows symbolic Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami symbolic.

---

### Brak powiadomień

symbolic Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla symbolic w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie symbolic Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji symbolic Desktop):

- `symbolic.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `symbolic.global.dat` i `symbolic.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z symbolic:
...
3. Upewnij się, że masz najnowszą wersję z `symbolic upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `symbolic models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\symbolic`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. symbolic dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\symbolic`

2. Uruchom ponownie kod symbolic, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to symbolic do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
symbolic wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
