...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji symbolic.
...
te lub skonfiguruj własne za pomocą konfiguracji symbolic.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.symbolic/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `symbolic.json`:
...
- Globalnie: `~/.config/symbolic/modes/`
...
Jeśli nie ma wpływu na temperaturę, symbolic stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna symbolic, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.symbolic/modes/` dla trybów zapisanych dla projektu lub `~/.config/symbolic/modes/` dla trybów globalnych:
