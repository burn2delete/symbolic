---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć symbolic.
---

Wtyczki umożliwiające wyłączenie symbolic poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania symbolic.
...
- `.symbolic/plugins/` - Wtyczki na poziomie projektu
- `~/.config/symbolic/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/symbolic/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/symbolic/symbolic.json`)
2. Konfiguracja projektu (`symbolic.json`)
3. Globalny katalog wtyczek (`~/.config/symbolic/plugins/`)
4. Katalog wtyczek projektu (`.symbolic/plugins/`)
...
symbolic uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z symbolic do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do symbolic.
...
:::note
Jeśli korzystasz z aplikacji komputerowej symbolic, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj symbolic czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do symbolic:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod symbolic. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://symbolic.computer/docs/sdk).
