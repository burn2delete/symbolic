...
W Symbolic obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Symbolic zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Symbolic ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

Symbolic ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `symbolic.json`:
...
- Globalnie: `~/.config/symbolic/agents/`
- W projekcie: `.symbolic/agents/`
...
Jeśli nie zostanie skonfigurowana, Symbolic używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Symbolic, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Symbolic w `provider/model-id`. Na przykład, [Symbolic Zen](/docs/zen), użyj `symbolic/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `symbolic models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/SymbolicOS/symbolic).
:::
