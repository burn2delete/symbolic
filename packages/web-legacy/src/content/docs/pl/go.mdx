...
Symbolic Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

:::note
Symbolic Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w Symbolic. Subskrybujesz Symbolic Go i
...
Symbolic Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.
...
1. Logujesz się do **<a href={console}>Symbolic Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `Symbolic Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Symbolic Go.
:::
...
Symbolic Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Symbolic
używa formatu `symbolic-go/<model-id>`. Na przykład dla Kimi K2.5 należy użyć
`symbolic-go/kimi-k2.5` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Symbolic.
