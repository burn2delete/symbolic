...
Symbolic Go to tania subskrypcja za **10 USD miesięcznie**, która zapewnia niezawodny dostęp do popularnych otwartych modeli kodowania.

:::note
Symbolic Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w Symbolic. Subskrybujesz Symbolic Go i otrzymujesz swój klucz API. Jest to **całkowicie opcjonalne** i nie musisz z tego korzystać, aby używać Symbolic.
...
Symbolic Go daje dostęp do tych modeli za **10 USD miesięcznie**.

---

## Jak to działa

Symbolic Go działa jak każdy inny dostawca w Symbolic.

1. Logujesz się do **<a href={console}>Symbolic Zen</a>**, subskrybujesz Go i kopiujesz swój klucz API.
2. Uruchamiasz polecenie `/connect` w TUI, wybierasz `Symbolic Go` i wklejasz swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych przez Go.

:::note
Tylko jeden członek na obszar roboczy może subskrybować Symbolic Go.
:::
...
Symbolic Go obejmuje następujące limity:
...
Symbolic Go to plan subskrypcji za **10 USD miesięcznie**. Poniżej znajdują się ceny **za 1 mln tokenów**.
...
[Identyfikator modelu](/docs/config/#models) w twojej konfiguracji Symbolic używa formatu `symbolic-go/<model-id>`. Na przykład dla Kimi K2.5 użyłbyś `symbolic-go/kimi-k2.5` w swojej konfiguracji.
...
4. Nie wprowadzać **żadnych blokad (lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy w Symbolic.
