...
Mit Symbolic kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Symbolic das `symbolic`-Theme.
...
Symbolic liefert mehrere eingebaute Themes mit.
...
- Symbolic optisch ans Terminal anpassen wollen
...
Symbolic unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/symbolic/themes/*.json` oder `$XDG_CONFIG_HOME/symbolic/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.symbolic/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.symbolic/themes/*.json`
