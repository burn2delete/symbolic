---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Symbolic zu erweitern.
---

Mit Plugins können Sie Symbolic erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Symbolic zu ändern.
...
- `.symbolic/plugins/` – Plugins auf Projektebene
- `~/.config/symbolic/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/symbolic/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/symbolic/symbolic.json`)
2. Projektkonfiguration (`symbolic.json`)
3. Globales Plugin-Verzeichnis (`~/.config/symbolic/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.symbolic/plugins/`)
...
Symbolic führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Symbolic-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Symbolic erweitern können.
...
:::note
Wenn Sie die Symbolic-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Symbolic `.env`-Dateien liest:
...
Plugins können Symbolic auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Symbolic aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Symbolic zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://symbolic.computer/docs/sdk).
