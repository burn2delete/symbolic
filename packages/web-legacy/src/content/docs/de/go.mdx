...
Symbolic Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

:::note
Symbolic Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Provider in Symbolic. Du abonnierst Symbolic Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Symbolic zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Providern getestet, die gut mit Symbolic funktionieren.
:::
...
Symbolic Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Symbolic Go funktioniert wie jeder andere Provider in Symbolic.

1. Du meldest dich bei **<a href={console}>Symbolic Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Symbolic Go` und fügst deinen API-Key ein.
...
Die [Modell-ID](/docs/config/#models) in deiner Symbolic Config verwendet das Format `symbolic-go/<model-id>`. Für Kimi K2.5 würdest du beispielsweise `symbolic-go/kimi-k2.5` in deiner Config verwenden.
...
Wir haben Symbolic Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Symbolic zu nutzen.
