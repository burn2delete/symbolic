...
const symbolicDir = path.join(repoDir, "packages", "symbolic")
...
const sandbox = await fs.mkdtemp(path.join(os.tmpdir(), "symbolic-e2e-"))
const keepSandbox = process.env.SYMBOLIC_E2E_KEEP_SANDBOX === "1"

const serverEnv = {
  ...process.env,
  SYMBOLIC_DISABLE_SHARE: process.env.SYMBOLIC_DISABLE_SHARE ?? "true",
  SYMBOLIC_DISABLE_LSP_DOWNLOAD: "true",
  SYMBOLIC_DISABLE_DEFAULT_PLUGINS: "true",
  SYMBOLIC_EXPERIMENTAL_DISABLE_FILEWATCHER: "true",
  SYMBOLIC_TEST_HOME: path.join(sandbox, "home"),
  XDG_DATA_HOME: path.join(sandbox, "share"),
  XDG_CACHE_HOME: path.join(sandbox, "cache"),
  XDG_CONFIG_HOME: path.join(sandbox, "config"),
  XDG_STATE_HOME: path.join(sandbox, "state"),
  SYMBOLIC_E2E_PROJECT_DIR: repoDir,
  SYMBOLIC_E2E_SESSION_TITLE: "E2E Session",
...
const runnerEnv = {
  ...
  VITE_SYMBOLIC_SERVER_HOST: "127.0.0.1",
  VITE_SYMBOLIC_SERVER_PORT: String(serverPort),
...
try {
  ...
  if (seedExit !== 0) {
    ...
    process.env.AGENT = "1"
    process.env.SYMBOLIC = "1"
    process.env.SYMBOLIC_PID = String(process.pid)

    const log = await import("../../symbolic/src/util/log")
    const install = await import("../../symbolic/src/installation")
    await log.Log.init({
      print: true,
    ...
    const servermod = await import("../../symbolic/src/server/server")
    inst = await import("../../symbolic/src/project/instance")
    server = servermod.Server.listen({ port: serverPort, hostname: "127.0.0.1" })
    console.log(`symbolic server listening on http://127.0.0.1:${serverPort}`)
