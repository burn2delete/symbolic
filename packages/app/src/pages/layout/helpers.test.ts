...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("symbolic://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("symbolic://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("symbolic://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("symbolic://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("symbolic://open-project")).toBeUndefined()
    expect(parseDeepLink("symbolic://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "symbolic://open-project?directory=/a",
      "symbolic://other?directory=/b",
      "symbolic://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("symbolic://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("symbolic://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("symbolic://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("symbolic://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "symbolic://new-session?directory=/a",
      "symbolic://open-project?directory=/b",
      "symbolic://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __SYMBOLIC__: {
        deepLinks: ["symbolic://open-project?directory=/a"],
      },
    } as unknown as Window & { __SYMBOLIC__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["symbolic://open-project?directory=/a"])
