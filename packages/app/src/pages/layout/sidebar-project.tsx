...
import { ContextMenu } from "@symbolic-agent/ui/context-menu"
...
export const SortableProject = (props: {
  ...
  const hovering = () => props.ctx.hoverProject() === props.project.worktree
  const preview = createMemo(() => !props.mobile && props.ctx.sidebarOpened())
  const overlay = createMemo(() => !props.mobile && !props.ctx.sidebarOpened())
  const active = createMemo(() => state.menu || (preview() ? hovering() : overlay() && hovering()))
  const hoverOpen = () => hovering() && preview() && !selected() && !state.menu
