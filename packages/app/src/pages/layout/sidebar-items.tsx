import type { Message, Session, TextPart, UserMessage } from "@symbolic-agent/sdk/v2/client"
import { Avatar } from "@symbolic-agent/ui/avatar"
import { HoverCard } from "@symbolic-agent/ui/hover-card"
import { Icon } from "@symbolic-agent/ui/icon"
import { IconButton } from "@symbolic-agent/ui/icon-button"
import { MessageNav } from "@symbolic-agent/ui/message-nav"
import { Spinner } from "@symbolic-agent/ui/spinner"
...
import { type Accessor, createEffect, createMemo, For, type JSX, Match, on, onCleanup, Show, Switch } from "solid-js"
...
const SYMBOLIC_PROJECT_ID = "4b0ea68d7af9a6031a7ffda7ad66e0cb83315750"
    ...
    <div class={`relative size-8 shrink-0 rounded ${props.class ?? ""}`}>
      <div class="size-full rounded overflow-clip">
        <Avatar
          fallback={name()}
          src={
            props.project.id === SYMBOLIC_PROJECT_ID ? "https://symbolic.computer/favicon.svg" : props.project.icon?.override
          }
  ...
  createEffect(
    on(
      ...
      (on, prev) => {
        clr()
        if (on) {
          setS({ open: true, show: true, fade: false })
          return
        }
        if (prev) {
          setS({ open: false, show: true, fade: true })
          t = window.setTimeout(() => setS({ show: false, fade: false }), 260)
          return
        }
        setS({ open: false, show: false, fade: false })
      },
      { defer: true },
    ),
  )
    ...
    >
      ...
      >
        ...
        <div
          ...
          classList={{
            "opacity-0": props.isWorking() || s.show,
          }}
        >
      ...
      <div class="flex min-w-0 items-center gap-1">
        <div class="min-w-0 flex-1">
          <Show
            when={hoverEnabled()}
            fallback={
              <Tooltip placement={props.mobile ? "bottom" : "right"} value={props.session.title} gutter={10} class="min-w-0 w-full">
                {item}
              </Tooltip>
            }
          >
