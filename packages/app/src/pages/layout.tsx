...
import { ResizeHandle } from "@symbolic-agent/ui/resize-handle"
...
import { DropdownMenu } from "@symbolic-agent/ui/dropdown-menu"
import { Dialog } from "@symbolic-agent/ui/dialog"
import { getFilename } from "@symbolic-agent/util/path"
import { Session, type Message } from "@symbolic-agent/sdk/v2/client"
...
import { showToast, Toast, toaster } from "@symbolic-agent/ui/toast"
...
import { useTheme, type ColorScheme } from "@symbolic-agent/ui/theme"
...
export default function Layout(props: ParentProps) {
  ...
  const [autoselecting] = createResource(
    ...
    async (value) => {
      ...
      if (!untrack(() => state.autoselect)) return false

      const list = value.list
      if (list.length === 0) {
        if (!value.last) return false
        setState("autoselect", false)
        openProject(value.last, false)
        navigateToProject(value.last)
        return true
      }

      const next = list.find((project) => project.worktree === value.last) ?? list[0]
  ...
  const SidebarPanel = (panelProps: { project: Accessor<LocalProject | undefined>; mobile?: boolean; merged?: boolean }) => {
