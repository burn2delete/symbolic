...
import { createSymbolic, type ToolPart } from "@symbolic-agent/sdk"
...
console.log("🚀 Starting symbolic server...")
const symbolic = await createSymbolic({
  port: 0,
})
console.log("✅ Symbolic server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await symbolic.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new symbolic session...")
    const { client, server } = symbolic
    ...
    console.log("✅ Created symbolic session:", createResult.data.id)
  ...
  console.log("📝 Sending to symbolic:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Symbolic response:", JSON.stringify(result, null, 2))
