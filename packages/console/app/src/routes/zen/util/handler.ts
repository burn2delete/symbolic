...
import { and, Database, eq, isNull, lt, or, sql } from "@symbolic-ai/console-core/drizzle/index.js"
import { KeyTable } from "@symbolic-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@symbolic-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@symbolic-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@symbolic-ai/console-core/util/date.js"
...
import { WorkspaceTable } from "@symbolic-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@symbolic-ai/console-core/model.js"
import { Subscription } from "@symbolic-ai/console-core/subscription.js"
import { BlackData } from "@symbolic-ai/console-core/black.js"
import { UserTable } from "@symbolic-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@symbolic-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@symbolic-ai/console-core/schema/provider.sql.js"
...
export async function handler(
  ...
  const ADMIN_WORKSPACES = [
    ...
    "wrk_01K6W1A3VE0KMNVSCQT43BG2SX", // symbolic bench
  ]
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-symbolic-session") ?? ""
    const requestId = input.request.headers.get("x-symbolic-request") ?? ""
    const projectId = input.request.headers.get("x-symbolic-project") ?? ""
    const ocClient = input.request.headers.get("x-symbolic-client") ?? ""
    logger.metric({
      is_tream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-symbolic-request")
          headers.delete("x-symbolic-session")
          headers.delete("x-symbolic-project")
          headers.delete("x-symbolic-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    const billingUrl = `https://symbolic.computer/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://symbolic.computer/workspace/${authInfo.workspaceID}/members`
