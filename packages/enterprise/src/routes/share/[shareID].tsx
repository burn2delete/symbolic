import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@symbolic-agent/sdk/v2"
import { SessionTurn } from "@symbolic-agent/ui/session-turn"
import { SessionReview } from "@symbolic-agent/ui/session-review"
import { DataProvider } from "@symbolic-agent/ui/context"
import { FileComponentProvider } from "@symbolic-agent/ui/context/file"
import { WorkerPoolProvider } from "@symbolic-agent/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@symbolic-agent/ui/logo"
import { IconButton } from "@symbolic-agent/ui/icon-button"
import { ProviderIcon } from "@symbolic-agent/ui/provider-icon"
...
import { Tabs } from "@symbolic-agent/ui/tabs"
import { MessageNav } from "@symbolic-agent/ui/message-nav"
import { FileSSR } from "@symbolic-agent/ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@symbolic-agent/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/symbolic-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Symbolic</Title>
              </Show>
              <Meta name="description" content="symbolic - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              ...
                              <IconButton
                                as={"a"}
                                href="https://symbolic.computer/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
