...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.symbolic.desktop.dev",
  beta: "ai.symbolic.desktop.beta",
  prod: "ai.symbolic.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.symbolic.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `symbolic.settings.dat` is special: it maps to the `symbolic.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"symbolic.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // symbolic.settings.dat → the electron settings store ("symbolic.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "symbolic.global.dat").
  const storeName = filename === "symbolic.settings.dat" ? "symbolic.settings" : filename
