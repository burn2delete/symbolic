...
const CLI_INSTALL_DIR = ".symbolic/bin"
const CLI_BINARY_NAME = "symbolic"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `symbolic-cli${suffix}`)
    : join(root, "../../resources", `symbolic-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "symbolic-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    SYMBOLIC_SERVER_USERNAME: "symbolic",
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    SYMBOLIC_EXPERIMENTAL_ICON_DISCOVERY: "true",
    SYMBOLIC_EXPERIMENTAL_FILEWATCHER: "true",
    SYMBOLIC_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.symbolic/bin/symbolic"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://symbolic.computer/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",
