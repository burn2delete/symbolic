...
app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Symbolic Dev")
app.setPath("userData", join(app.getPath("appData"), app.isPackaged ? APP_IDS[CHANNEL] : "ai.symbolic.desktop.dev"))
...
function setupApp() {
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("symbolic://"))
    if (urls.length) {
  ...
  void app.whenReady().then(async () => {
    // migrate()
    app.setAsDefaultProtocolClient("symbolic")
    setDockIcon()
    setupAutoUpdater()
    syncCli()
    await initialize()
...
async function getSidecarPort() {
  const fromEnv = process.env.SYMBOLIC_PORT
  if (fromEnv) {
...
function sqliteFileExists() {
  ...
  return existsSync(join(base, "symbolic", "symbolic.db"))
}
