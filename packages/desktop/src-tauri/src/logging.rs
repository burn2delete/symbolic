...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("symbolic-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("symbolic_lib=debug,symbolic_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("symbolic_lib=info,symbolic_desktop=info,sidecar=info")
        }
    });
