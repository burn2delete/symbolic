...
import { createSymbolicClient } from "@symbolic-ai/sdk"
...
const { client, server } = createSymbolic()
...
function createSymbolic() {
  ...
  const proc = spawn(`symbolic`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createSymbolicClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)\/symbolic(?=$|\s)/)) {
    throw new Error("Comments must mention `/symbolic`")
  }
}
...
async function assertSymbolicConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to symbolic server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.symbolic.computer" : "https://symbolic.computer"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.symbolic.computer/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("symbolic-github-action")
    response = await fetch("https://api.symbolic.computer/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "symbolic-agent[bot]"`
  await $`git config --global user.email "symbolic-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `symbolic/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/symbolic-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[symbolic session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
