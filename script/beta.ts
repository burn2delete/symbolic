...
const model = "symbolic/gpt-5.3-codex"
...
async function typecheck() {
  ...
  try {
    await $`bun typecheck`.cwd("packages/symbolic")
    return true
  } catch (err) {
...
async function build() {
  ...
  try {
    await $`bun run script/build.ts --single --skip-install`.cwd("packages/symbolic")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with symbolic...`)
  ...
  const prompt = [
    ...
    "If a PR already deleted a file or directory, do not re-add it; apply changes in the current semantic location instead.",
    "If a PR already changed an import, keep that change.",
    "After resolving the conflicts, run `bun typecheck` in `packages/symbolic`.",
  ...
  try {
    await $`symbolic run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  symbolic failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with symbolic")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with symbolic...")

  const done = lines(prs.filter((x) => applied.includes(x.number)))
  const prompt = [
    "The beta merge batch is complete.",
    `Merged PRs on HEAD:\n${done}`,
    "Run `bun typecheck` in `packages/symbolic`.",
    "Run `bun run script/build.ts --single --skip-install` in `packages/symbolic`.",
